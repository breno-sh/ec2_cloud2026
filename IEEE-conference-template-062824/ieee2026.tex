...
\definecolor{shadecolor}{RGB}{255,255,200}
\newcommand{\revnote}[1]{\todo[inline, color=green!40, size=\footnotesize]{#1}}
...
\begin{document}
	...
	Cloud-based video processing has become fundamental to modern media delivery, supporting streaming platforms, surveillance systems, and large-scale content distribution. However, achieving cost-effective performance in cloud environments presents significant challenges, as diverse instance types create complex trade-offs between computational capacity, scalability, and operational expenses \cite{b1,b17}. \hl{While recent standards like Versatile Video Coding (VVC) offer improved compression, their extreme computational cost currently makes them unsuitable for minimal burstable instances. Thus, we focus on H.264, H.265, and VP9, which represent the vast majority of commercial workloads.}\revnote{Clarified exclusion of VVC due to high comp. cost, focusing on commercial dominance of H.264/HEVC/VP9 (Response to R1).}
	...
	\begin{shaded}
	\subsection{CPU Credit Mechanics}
	\label{met:cpu-credits}
    \revnote{New Section: Explicitly explained T-instance "Unlimited Mode" and credit sustainability for batch jobs (Response to R3).}

	AWS T3 instances operate in \textit{Unlimited Mode} by default, allowing burst performance beyond baseline (10\% for t3.micro) by consuming CPU credits. Each instance earns credits at a baseline rate when idle and spends them during burst periods.

	Our experimental design ensures credit sustainability for batch workloads:
	\begin{itemize}
		\item \textbf{Ephemeral Pattern:} Instances spawn, encode, and terminate within 1.45 minutes (Phase 3.2), keeping 24-hour average utilization low.
		\item \textbf{Credit Balance:} Short jobs ($<2$ min) complete using initial burst capacity without depleting accumulated credits.
		\item \textbf{Surplus Avoidance:} By minimizing provisioning time via AMI optimization (Section~\ref{results-phase3}), we reduce the window where instances consume credits at full CPU.
	\end{itemize}

	This validates that for divisible batch workloads (VOD transcoding, user-generated content processing), T-family instances remain cost-effective without incurring surplus credit charges.
	\end{shaded}
	...
	\begin{shaded}
	\subsubsection{Workload Distribution and Failure Handling}
    \revnote{Added details on Split/Merge pipeline and Failure Granularity (Response to R3).}

	The 10-minute source video is segmented into 10 one-minute chunks using FFmpeg's \texttt{segment} muxer: \texttt{-f segment -segment\_time 60}. These segments are distributed to a pool of 10 t3.micro instances via parallel SSH (pSSH) with a round-robin scheduler. Each instance processes its assigned segment independently using the containerized FFmpeg environment described in Section~\ref{execution-initial-performance-benchmarks}.
	
	Upon completion, outputs are transferred to a central S3 bucket and concatenated using FFmpeg's \texttt{concat} demuxer. This architecture provides \textit{failure granularity}: if a single instance fails during processing (e.g., minute 7 of 10), only that 60-second segment requires retry ($\sim$6s recomputation), compared to a vertical scaling failure where 90+ seconds of processing would be lost.
	\end{shaded}
	...
	\begin{shaded}
	Equation~\ref{eq:efficiency} defines Efficiency $E$ as the inverse of the cost-time product. This metric specifically identifies the ``sweet spot'' where neither cost nor latency dominates unfavorably:
	
	\begin{itemize}
		\item \textbf{Scenario A:} High-cost, fast instance (\$0.01, 10s) $\rightarrow E = 10$
		\item \textbf{Scenario B:} Low-cost, slow instance (\$0.001, 100s) $\rightarrow E = 10$
	\end{itemize}
	
	While Scenario B is 10$\times$ cheaper per hour, both achieve equal efficiency because B's cost advantage is offset by 10$\times$ runtime. This differs from ``work per dollar'' (which ignores latency constraints) or pure speedup (which ignores budget), making $E$ suitable for latency-sensitive divisible workloads.
	\revnote{Added intuition for Efficiency metric (Response to R2).}
	\end{shaded}
	...
	\begin{table*}[h]
		\centering
		\caption{Descriptive statistics for compression time (Time) efficiency (Eff.), and total cost. Best results highlighted in \textbf{bold}.}
		...
		\resizebox{\textwidth}{!}{
			\begin{tabular}{ll|rrrrr|rrrrr|rrrrr}
				...
				\midrule
				t2.micro & 1 thread & 2.6350 & 0.0125 & 2.6125 & 2.6625 & 117,678 | 0.0000085 & 5.0727 & 0.0259 & 5.0227 & 5.1136 & 61,185 | 0.0000163 & 15.1205 & 0.0559 & 15.0461 & 15.2922 & 20,551 | 0.0000486 \\
				t2.micro & 3 thread & 2.7766 & 0.0211 & 2.7433 & 2.8485 & 111,949 | 0.0000089 & 5.0227 & 0.0293 & 4.9592 & 5.1073 & 61,863 | 0.0000162 & 14.7580 & 0.0470 & 14.7042 & 14.9585 & \textbf{21,059} | 0.0000475 \\
				...
				t3.micro & 5 thread & 2.5307 & 0.0677 & 2.2560 & 2.7556 & \textbf{137,352} | 0.0000073 & 4.3159 & 0.0299 & 4.2833 & 4.3765 & 80,471 | 0.0000125 & 17.5244 & 0.2950 & 17.3751 & 18.7260 & 19,897 | 0.0000507 \\
				t3.micro & 10 thread & 2.5702 & 0.0196 & 2.4820 & 2.6076 & 135,274 | 0.0000074 & 4.2952 & 0.1018 & 3.7388 & 4.3470 & \textbf{80,955} | 0.0000124 & 17.5015 & 0.1475 & 17.3627 & 17.9877 & 19,929 | 0.0000506 \\
				\midrule
				...
				t3.2xlarge & 5 thread & \textbf{1.4430} & 0.0550 & 1.2975 & 1.5330 & 7,542 | 0.0001326 & 2.0833 & 0.0936 & 1.9711 & 2.3515 & 5,219 | 0.0001916 & 17.3835 & 0.0448 & 17.3013 & 17.4682 & 623 | 0.0016060 \\
				t3.2xlarge & 10 thread & 1.5297 & 0.0365 & 1.4588 & 1.6326 & 7,058 | 0.0001417 & \textbf{1.8286} & 0.0601 & 1.7110 & 1.9614 & 5,935 | 0.0001685 & \textbf{17.2667} & 0.0377 & 17.2049 & 17.3305 & 627 | 0.0015941 \\
				\bottomrule
			\end{tabular}
		}
	\end{table*}
...
\revnote{New Section: Limitations (Address R2/R3 scope concerns).}

While our findings demonstrate significant cost-efficiency improvements, several limitations scope the generalizability of results:

\textbf{Infrastructure Scope:} Experiments were conducted exclusively in AWS \texttt{us-east-1} using T-family (t2/t3) x86 instances. Results may not transfer directly to: (a) ARM-based instances (e.g., AWS Graviton), (b) GPU-accelerated instances (p3/g4dn), (c) Compute-optimized families (c5/c6), or (d) other cloud providers with different pricing models or hardware heterogeneity profiles.

\textbf{Workload Characteristics:} We utilized 1080p (Full HD, 30fps) test sequences with mixed motion profiles encoded at 2Mbps using the \texttt{slow} preset. Higher resolutions (4K), high-motion sports content, or low-latency presets may alter the performance-cost balance.

\textbf{Cost Analysis Scope:} Our economic analysis considers only EC2 compute costs. In production pipelines, storage (S3), data transfer (egress), and API request costs may impact total cost of ownership (TCO), though these scale linearly regardless of instance selection strategy.

\textbf{CPU Credit Dynamics:} T-family instances rely on burst CPU credits. Our workloads represent \textbf{short batch jobs} (mean duration < 2 minutes) that complete within initial credit balances using Unlimited Mode. Sustained 24/7 encoding would trigger surplus credit charges, potentially altering cost calculations.

\textbf{Failure and Retry Handling:} While our architecture supports granular retry (1-minute segments vs. full 10-minute video), we did not inject artificial failures to measure recovery overhead under stress conditions.
\end{shaded}
