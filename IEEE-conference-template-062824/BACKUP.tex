...
\begin{document}
	...
	\author{
		...
		\IEEEauthorblockA{For Double-Blind Review}
	}
	...
	Cloud computing, particularly Infrastructure as a Service (IaaS), has revolutionized the way businesses deploy and manage computing resources. Amazon EC2, one of the most widely used IaaS platforms, offers a diverse range of instance types, flexible configurations, and global availability. However, this flexibility introduces significant cost and performance optimization challenges in selecting the optimal instance type for video compression workloads \cite{Maas2025}. The substantial number of instance configurations, coupled with varying price models, complicates resource allocation decisions \cite{mahida-ankur}.
	
	For many organizations processing video content, the choice of EC2 instance has implications for both performance and cost efficiency \cite{p__praveenchandar_6ddf409a, tamara_dancheva_635e0512}. This study focuses specifically on the AWS T-family of instances (e.g., t2, t3). These instances are designed as a low-cost, general-purpose option that provides a baseline level of CPU performance with the ability to "burst" to a higher level using a system of CPU credits. A key characteristic that makes them compelling for on-demand tasks is that a newly launched instance starts with a sufficient credit balance to burst immediately. This enables an architectural pattern where instances are treated as disposable workers: they are provisioned for a single, short, CPU-intensive task -- like compressing a 30-second video segment -- and then terminate. This approach leverages their full burst capacity without the risk of credit depletion over time. Our research is motivated by the need to quantify the cost-effectiveness of this strategy, determining the "sweet spot" where the low cost of T-family instances outweighs the raw power of more expensive, fixed-performance alternatives.
	
	\subsection{Research Context and Scientific Contributions}
	
	While significant progress has been made in cloud resource optimization, certain aspects of cost-efficiency for media processing workloads remain underexplored. In particular, the trade-offs between horizontal and vertical scaling for video encoding tasks, and the impact of operational overhead on parallel architectures, require further investigation.
	
	This paper extends the current understanding by making the following contributions:
	
	\begin{enumerate}
		\item \textbf{Empirical Validation of the Small Instance Efficiency Paradox:} We provide systematic evidence challenging the conventional wisdom that larger cloud instances are inherently more cost-effective. Through systematic benchmarking across 2,520 compression operations, we demonstrate that smaller burstable instances can achieve up to \textbf{20× higher cost-efficiency} despite longer absolute processing times.
		
		\item \textbf{Quantification of Provisioning Overhead in Parallel Architectures:} Environment setup consumed \textbf{70\% of total processing time} in unoptimized horizontally-scaled video encoding workflows.
		
		\item \textbf{AMI-Based Optimization Framework:} We develop and validate a practical optimization strategy using custom machine images that reduces parallel processing time by \textbf{55.9\%} and costs by \textbf{55.8\%}, demonstrating that proper initialization can transform the economic calculus between scaling strategies.
		
		\item \textbf{Horizontal vs. Vertical Scaling Trade-off Analysis:} Through a two-phase case study, we provide empirical evidence that properly optimized horizontal scaling can simultaneously outperform vertical scaling in both processing time (\textbf{1.28× faster}) and cost (\textbf{75.5\% reduction}) for divisible video workloads.
	\end{enumerate}
	
	\subsection{Research Objectives and Methodology}
	
	The primary motivation for this study is motivated by the need to understand these performance characteristics, with the ultimate goal of enabling more informed and cost-effective instance selection. This research addresses three key objectives:
	\begin{enumerate}
		\item Measure and compare the video compression performance of various EC2 T-family instance types, including t2.micro, t3.micro, t3.small, t3.medium, t3.large, t3.xlarge, and t3.2xlarge.
		\item Investigate the impact of different video encoding codecs (H.264, H.265, and VP9) on the performance of these instances.
		\item Propose more accurate and cost-effective resource allocation decisions based on comprehensive video compression performance evaluation.
	\end{enumerate}
	
	This study investigates the video compression performance of various Amazon EC2 instance types, building on prior work by \cite{tamara_dancheva_635e0512}. Understanding these performance variations is essential for efficient resource allocation and cost-effective deployment, helping organizations make informed decisions about instance selection to ensure both computational efficiency and cost savings for video processing workloads.
	
	Previous studies have consistently reported hardware heterogeneity in cloud environments \cite{pedro__lvarez_ed947120, zhonghong_ou_93958397, zhonghong_ou_2f9f325e, benjamin_farley_32066261}. Our preliminary experiments reinforce these findings, indicating that such heterogeneity still persists, particularly across availability zones within the us-east-1 region for certain EC2 instance types. We launched 100 instances of each type in each availability zone (a, b, c, d, e, f) and executed the \texttt{cat /proc/cpuinfo} command on each instance to retrieve detailed processor information. The results revealed that, in some zones, instances of the same type were provisioned with different CPU models, confirming the presence of hardware heterogeneity (see more in section \ref{met:validation-processor-homogeneity}).
	
	To evaluate video compression performance, we conducted extensive tests using three popular codecs (H.264 -- aka Advanced Video Coding (AVC), H.265 -- aka High-Efficiency Video Coding (HEVC), and VP9) on the selected EC2 instance types. Performance metrics were recorded and analyzed to characterize the compression performance of each type of instance. These findings provide insights for optimizing instance selection and resource allocation for video processing in cloud environments.
	
	By addressing these objectives, we aim to provide a better understanding of the video compression performance characteristics of EC2 instance types and enable more informed and efficient resource allocation decisions for video processing workloads in cloud computing environments.
	
	\subsection{Paper Organization}
	
	This paper is organized as follows. Section~\ref{rel_works} reviews related work on cloud resource optimization and video processing, highlighting challenges in cost-performance trade-offs and the importance of standardized benchmarking. Section~\ref{met} details our research methodology, organized into three sequential phases: Validation of Processor Homogeneity (3.1), Initial Performance Benchmarks (3.2), and Horizontal vs. Vertical Scaling Analysis (3.3). Section~\ref{experiments} presents results corresponding to each methodological phase, with subsections 4.1, 4.2, and 4.3 following the same structure. Finally, Section~\ref{conclusion} concludes the paper, summarizing our key contributions and outlining directions for future work.
	
	\section{Related Works}
	
	\label{rel_works}
	
	Video processing computational resource optimization presents research challenges due to modern codec complexity. Advanced standards like Versatile Video Coding (VVC) introduce sophisticated tools that increase computational demands, driving research into algorithmic optimizations \cite{rosado2024coding, araujo2024fast}. While prior work focuses on algorithmic complexity, we investigate cloud infrastructure performance and cost-effectiveness for practical deployment.
	
	Cloud resource optimization has been studied from multiple perspectives. Prior work includes cost-driven provisioning methodologies for scientific computing \cite{pedro__lvarez_ed947120} and surveys categorizing resource management strategies \cite{lailan_m_haji_2b4efda5}. Our research extends this foundation by demonstrating that properly optimized horizontal architectures can simultaneously outperform vertical scaling for video encoding workloads in both processing speed and cost-efficiency.
	
	Recent work by Jiang et al. \cite{jiang2025cloud} introduces PIVOT, a cloud-agnostic framework that addresses cost-aware scheduling in multi-cloud environments. Their approach tackles significant challenges in cross-cloud data analysis, including infrastructure heterogeneity, cross-cloud data movement limitations, and prohibitive egress costs. The PIVOT framework employs a two-level scheduling architecture with a cost-aware algorithm that minimizes total expenses (VM subscription + egress network traffic) while maintaining application performance. Their results demonstrate up to 90.8\% savings in VM subscription costs and 99.2\% reduction in egress traffic costs compared to opportunistic scheduling approaches. While PIVOT focuses on multi-cloud workflow orchestration for big data applications, our work complements this by providing granular analysis of cost-performance trade-offs within a single cloud provider's burstable instance family, specifically for video encoding workloads. Both approaches share the fundamental goal of cost optimization but address different layers of the cloud resource management stack.
	
	The multi-objective nature of cloud optimization problems has been well-established in literature. Studies such as \cite{angeliki_katsenou_e0a9a00f} provide thorough analyses of energy efficiency trade-offs across different codecs and hardware platforms. Conversely, recent comprehensive analyses like \cite{Maas2025} offer detailed investigations of cost-performance balances across diverse cloud instances. Our work aligns with this latter approach but introduces a specialized focus on burstable instance families and parallel encoding architectures, with particular emphasis on quantifying operational overheads that are frequently overlooked in theoretical models.
	
	Benchmarking methodologies represent a dimension of cloud performance research. Comparative studies illustrate heterogeneity in cloud technologies, highlighting challenges in establishing direct performance comparisons \cite{omar_alzakholi_0537f875, Maas2025}. Our methodology addresses this challenge through environmental standardization using containerized execution environments, aligning with approaches employed in studies such as \cite{arnaldo_pereira_ferreira_bd8560e5} to ensure consistent and reproducible experimental conditions.
	
	Our research addresses cloud performance variability through empirical measurement. We quantify variability across 30 trials per configuration, establishing baseline performance profiles for cost-efficiency analysis, complementing predictive modeling approaches \cite{joel_scheuner_a07bc84e, sepideh_ebneyousef_1d5c43ab}.
	
	The architectural principles underlying our parallel processing case study draw from established distributed systems concepts. The fundamental strategy of decomposing large, divisible workloads to optimize cost efficiency has been demonstrated in contexts such as task scheduling in fog-cloud environments \cite{tina_samizadeh_nikoui_765428f4, rumr2003}. Our contribution lies in applying these principles to a specialized yet practically significant domain: parallel video encoding within public cloud infrastructure, with particular emphasis on quantifying and mitigating provisioning overhead.
	...
	\begin{table*}[!htbp]
		...
		\begin{tabular}{|p{0.15\linewidth}|p{0.18\linewidth}|p{0.28\linewidth}|p{0.28\linewidth}|}
			...
			\hline
			Álvarez et al. & Scientific workloads & Cost-aware provisioning & No media processing focus \\
			\hline
			Dancheva et al. & Video encoding performance & Encoding time comparison & Limited cost analysis \\
			\hline
			Jiang et al. & Multi-cloud scheduling & Cost-aware framework for cross-cloud workflows & Big data focus, limited video processing analysis \\
			\hline
			Maas et al. & Broad cloud benchmarking & Instance performance analysis & No video encoding specialization \\
			\hline
			Ferraz et al. & Benchmarking methodologies & Reproducible frameworks & No video encoding application \\
			\hline
			Katsenou et al. & Energy efficiency & Codec energy analysis & Embedded devices focus \\
			\hline
	...
	\begin{lstlisting}[language=bash, frame=single, basicstyle=\ttfamily\small]
		ffmpeg -i input.mp4 -c:v <codec> -preset slow -b:v 2M -threads <n> output.mp4
	\end{lstlisting}
	...
	\begin{table*}[t]
		...
		\begin{tabular}{|l|c|c|c|c|}
			...
			\hline
			Minimal (t3.micro) & 2 & 1 & 0.0104 & Horizontal Scaling / Baseline \\
			\hline
	...
	For the purposes of this study, the `cost` variable refers to the hourly cost for each instance type, as shown in Table~\ref{tab:instance_costs}, and `time` is the compression time measured in seconds. The Efficiency metric represents a normalized measure that integrates performance with infrastructure cost, enabling direct comparison across instance types, while Total Cost provides the absolute monetary expenditure per operation for practical budgeting and decision-making.
	
	\begin{table}[htbp]
		\centering
		\caption{Hourly costs for the EC2 instances used in this study (On-Demand pricing, us-east-1). These values form the basis for our efficiency and cost calculations.}
	...
	This investigation has presented a comprehensive performance and cost-efficiency analysis of computational instances for video encoding workloads, culminating in an experimentally validated framework for optimizing parallel video processing architectures in cloud environments.
	
	\subsection{Summary of Empirical Findings}
	
	Our experimental results demonstrate that while vertical scaling provides straightforward performance improvements, strategically optimized horizontal architectures deliver superior performance and economic efficiency:
	
	\begin{itemize}
		\item \textbf{Cost-Efficiency Paradox:} Smaller burstable instances (t3.micro) demonstrated up to \textbf{20× higher cost-efficiency} despite longer absolute processing times, challenging conventional scaling assumptions.
		
		\item \textbf{Provisioning Overhead Impact:} Environment setup consumed \textbf{70\% of total processing time} in unoptimized parallel deployments.
		
		\item \textbf{Optimized Horizontal Scaling Superiority:} Through systematic overhead elimination using pre-configured machine images, a cluster of ten t3.micro instances achieved processing in \textbf{1.45 minutes}—\textbf{1.28× faster} than a single powerful instance (1.85 minutes) while achieving \textbf{75.5\% cost reduction}.
	\end{itemize}
	
	\subsection{Theoretical and Practical Implications}
	
	Our findings yield several significant implications for both research and practice in cloud-based media processing:
	
	\subsubsection{Theoretical Contributions}
	\begin{itemize}
		\item \textbf{Resource Allocation Models:} Challenges conventional scaling assumptions by demonstrating that raw computational power does not necessarily correlate with economic efficiency for specific workload categories.
		\item \textbf{Benchmarking Methodologies:} Establishes a systematic framework for quantifying provisioning overhead—a frequently neglected yet substantial factor in distributed system performance.
		\item \textbf{Cloud Architecture Principles:} Provides empirical evidence that properly optimized horizontal scaling can simultaneously outperform vertical scaling in both performance and cost for divisible workloads.
	\end{itemize}
	
	\subsubsection{Practical Recommendations}
	\begin{itemize}
		\item \textbf{Adopt Pre-Configured Image Strategies:} Dynamic environment provisioning constitutes the primary performance bottleneck in parallel encoding architectures. Utilization of pre-configured machine images should be considered essential rather than optional.
		\item \textbf{Prioritize Horizontal Scaling for Partitionable Workloads:} For computationally divisible tasks like batch video processing, scale-out architectures utilizing multiple cost-optimized instances demonstrably outperform scale-up approaches.
		\item \textbf{Implement Workload-Specific Instance Selection:} Computational instance selection should be driven by primary operational objectives—minimal instances for cost-efficiency, high-performance instances for latency-sensitive tasks.
	\end{itemize}
	
	\subsection{Limitations and Future Research Directions}
	
	This investigation identifies several promising avenues for future research:
	
	\begin{itemize}
		\item \textbf{Extended Workload Characterization:} Application of the established methodological framework to additional media processing workloads including video transcoding, image processing, and audio encoding.
		\item \textbf{Intelligent Scaling Frameworks:} Development of dynamic instance count adaptation based on real-time workload characterization and performance monitoring.
		\item \textbf{Cost-Optimized Instance Integration:} Exploration of fault-tolerant architectural patterns to leverage spot instances and preemptible resources for additional cost reductions.
		\item \textbf{Performance Variability Investigation:} Root cause analysis of performance fluctuations in minimal computational instances and cold start mitigation strategies.
	\end{itemize}
	
	This research establishes a foundation for more efficient cloud resource utilization in media processing applications, contributing to both theoretical understanding and practical implementation of cost-optimized computational architectures.
	...
	\begin{thebibliography}{00}
		...
		\bibitem{b19} P. H. G. Silva, ``Análise de precificação de instâncias SPOT para minimização de custos em um contexto multi-nuvem,'' M.S. dissertation, Univ. Federal Pernambuco, Recife, Brazil, 2025.
		
		\bibitem{b20} F. Jiang, K. Ferriter, and C. Castillo, ``A Cloud-Agnostic Framework to Enable Cost-Aware Scheduling of Applications in a Multi-Cloud Environment,'' in \emph{IEEE/IFIP NOMS}, 2025.
