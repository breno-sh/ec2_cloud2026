...
\begin{document}
	...
	This study investigated video compression performance using H.264, H.265, and VP9 codecs on AWS EC2 instances, varying thread counts. The metrics evaluated are Compression Time (seconds), Efficiency (Equation~\ref{eq:efficiency}), and Total Cost per operation.
	...
	The `cost` variable refers to hourly instance cost from Table~\ref{tab:experimental_configurations}, and `time` is compression time in seconds. Efficiency integrates performance with infrastructure cost, while Total Cost provides monetary expenditure per operation.
	
	\subsubsection{Performance Analysis}
	
	Table \ref{tab:final_comparison_consistent} presents descriptive statistics for Compression Time, Efficiency, and Total Cost across all experimental configurations. The data reveal several patterns that extend beyond the expected finding that more powerful instances complete tasks faster.
	...
	Analysis of Table~\ref{tab:final_comparison_consistent} reveals several patterns:
	
	\textbf{Cost-Efficiency Analysis:} While the \texttt{t3.2xlarge} instance shows the fastest absolute performance (1.443 seconds for H.264 with 5 threads), the \texttt{t3.micro} instance delivers superior cost-efficiency, processing the same task for \$0.0000073 compared to \$0.0001326 — approximately 18 times more cost-effective.
	
	\textbf{Thread Scaling Patterns:} Results show diminishing returns with increased thread counts across all instance types and codecs. Performance improvements plateau beyond 3-5 threads, with minimal gains at 10 threads. This saturation can be attributed to:
	\begin{enumerate}
		\item \textbf{Amdahl's Law Limitations:} Video encoding contains sequential components that cannot be parallelized
		\item \textbf{Memory Bandwidth Constraints:} Smaller instances face memory bandwidth limitations with multiple threads
		\item \textbf{FFmpeg Parallelization Architecture:} Pipeline stages with dependencies limit perfect parallelization
	\end{enumerate}
	
	\textbf{Codec-Specific Performance:} H.265 encoding shows consistent improvements with larger instances, while VP9 demonstrates less sensitivity to instance size, suggesting different parallelization characteristics.
	...
	These results confirm that instance selection requires balancing performance needs with cost efficiency.
	
	\subsubsection{Visual Analysis of Results}
	
	\textbf{Performance Comparison Across Codecs:} Figures~\ref{fig:faceted_bar_charts_h264}, \ref{fig:faceted_bar_charts_h265} and \ref{fig:faceted_bar_charts_vp9}, presents faceted bar charts comparing median compression time across all instances and thread configurations. The charts illustrate diminishing returns from additional threads, particularly on smaller instances, and highlight which instance types benefit most from parallelization.
	
	% Figura H.264
	\begin{figure}[!h]
		\centering
		\includegraphics[width=\linewidth]{graphs/bar_chart_time_h264.png}
		\caption{H.264 Codec Performance: Comparative analysis of median compression time across instance configurations with varying thread counts.}
	...
	\begin{figure}[!h]
		...
		\caption{H.265 Codec Performance: Comparative analysis of median compression time across instance configurations with varying thread counts.}
...
\includegraphics[width=\linewidth]{graphs/bar_chart_time_vp9.png}
		\caption{VP9 Codec Performance: Comparative analysis of median compression time across instance configurations with varying thread counts.}
	...
	\textbf{Hierarchical Efficiency Overview:} Figure~\ref{fig:sunburst_efficiency_normalized_overall} presents a normalized Sunburst chart providing a hierarchical overview of encoding efficiency across all experimental configurations. The visualization enables clear comparison of instance and thread configurations within each algorithmic context.
	

	
	\textbf{Algorithm-Specific Efficiency Analysis:} Figures~\ref{fig:h264_heatmap}, \ref{fig:h265_heatmap} and \ref{fig:vp9_heatmap} presents a series of heatmaps visualizing median efficiency for each algorithm individually. Each heatmap correlates thread count with instance type, with color intensity representing cost-efficiency.
...
\begin{figure}[H]
	...
	\caption{H.264 Codec Efficiency: Heatmap visualization correlating thread count (vertical axis) with instance type (horizontal axis). Color intensity represents cost-efficiency, with optimal configurations emerging as regions of maximum intensity.}
...
\end{table*}
	...
	Based on findings identifying \texttt{t3.micro} as most cost-effective for H.264 compression, we compared scaling strategies using a 10-minute video workload:
	
	\begin{itemize}
		\item \textbf{Horizontal Scaling:} 10 \texttt{t3.micro} instances processing one-minute segments in parallel
	...
	\textbf{Phase 3.1: Dynamic Provisioning} (standard Ubuntu images with post-deployment installation):
	\begin{itemize}
		...
		\item Horizontal approach demonstrated cost advantages: \$0.00568 vs. \$0.01687 (66.3\% reduction)
	\end{itemize}
	
	\textbf{Phase 3.2: Optimized Provisioning} (custom AMIs with pre-installed dependencies):
	\begin{itemize}
		\item Horizontal scaling achieved 1.45 minutes total time — 55.9\% improvement over Phase 3.1 and 1.28× faster than vertical scaling (1.85 minutes)
		\item Costs reduced by 55.8\% to \$0.00251, achieving 75.5\% cost reduction compared to vertical scaling
		\item Setup time decreased from 2.30 minutes to 0.98 minutes (57.4\% reduction)
	\end{itemize}
	
	\subsubsection{Analysis and Validation}
	
	Table~\ref{tab:unified_case_study} presents the comprehensive comparison of scaling strategies across optimization phases, showing that properly optimized horizontal scaling achieves better performance and cost-efficiency.
	
	
	
	Figure~\ref{fig:ami_comparison} demonstrates that horizontal scaling achieves superior performance and cost-efficiency when provisioning overhead is minimized through AMI optimization.
	...
	The experimental design provides validation through:
	\begin{enumerate}
		\item Controlled comparison of identical workloads across scaling strategies
		\item Quantification of provisioning overhead (70.2\% of total time in unoptimized deployments)
		\item Multiple experimental runs (n=30 per configuration) for statistical confidence
		\item Practical workload (10-minute video) and optimization strategy (custom AMIs)
	\end{enumerate}
	
	The results demonstrate that horizontal scaling's theoretical advantages can be realized when operational bottlenecks are addressed. The 75.5\% cost reduction and 1.28× performance improvement provide evidence for re-evaluating vertical scaling approaches for video encoding workloads.
	...
	The cost variable corresponds to hourly instance rates from Table~\ref{tab:experimental_configurations}, while time represents compression duration in seconds. The Efficiency metric normalizes performance against infrastructure expenditure, enabling cross-instance comparison, whereas Total Cost quantifies absolute monetary outlay per operation.
	
	\subsubsection{Performance Analysis}
	
	Table \ref{tab:final_comparison_consistent} presents comprehensive descriptive statistics for Compression Time, Efficiency, and Total Cost across all experimental configurations. The analysis reveals performance patterns extending beyond the expected correlation between instance capability and processing speed.
