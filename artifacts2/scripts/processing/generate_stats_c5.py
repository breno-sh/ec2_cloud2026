...
csv_files = [
    'aggregated_c5_large.csv',
    'aggregated_c5_xlarge.csv',
    'aggregated_c5_2xlarge.csv',
    'aggregated_c5_4xlarge.csv',
]

instance_costs_per_hour = {'c5_large': 0.085, 'c5_xlarge': 0.170, 'c5_2xlarge': 0.340, 'c5_4xlarge': 0.680}
...
for (instance_type, metric_name), values in all_data.items():
    ...
    for value in values:
        ...
        data_for_analysis.append({'InstanceType': instance_type, 'Codec': codec, 'Threads': threads, 'CompressionTime': value, 'Efficiency': efficiency, 'TotalCost': total_cost})

df_analysis = pd.DataFrame(data_for_analysis)
desc_stats = df_analysis.groupby(['InstanceType', 'Codec', 'Threads']).agg(
    CompressionTime_Median=('CompressionTime', 'median'), CompressionTime_Std=('CompressionTime', 'std'), CompressionTime_Min=('CompressionTime', 'min'), CompressionTime_Max=('CompressionTime', 'max'), Efficiency_Median=('Efficiency', 'median'), TotalCost_Median=('TotalCost', 'median')).reset_index()

instance_order = ['c5_large', 'c5_xlarge', 'c5_2xlarge', 'c5_4xlarge']
...
desc_stats.to_csv(os.path.join(OUTPUT_DIR, 'descriptive_statistics_c5.csv'), index=False)
table_rows = []
...
md_lines = []
md_lines.append("# Descriptive Statistics - C-Family Instances\n")
...
for row in table_rows:
    ...
    for codec in codec_order:
        if f"{codec}_median" in row:
            vals.extend([f"{row[f'{codec}_median']:.4f}", f"{row[f'{codec}_std']:.4f}", f"{row[f'{codec}_min']:.4f}", f"{row[f'{codec}_max']:.4f}", f"{row[f'{codec}_eff']:,.0f}", f"${row[f'{codec}_cost']:.7f}"])
        else:
            vals.extend(["-", "-", "-", "-", "-", "-"])
    md_lines.append("| " + " | ".join(vals) + " |")

with open(os.path.join(OUTPUT_DIR, 'descriptive_statistics_c5.md'), 'w') as f:
    f.write("\n".join(md_lines))
