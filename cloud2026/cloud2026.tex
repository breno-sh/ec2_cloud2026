...
\begin{abstract}
As Video-as-a-Service workloads increasingly dominate cloud media delivery, selecting the right EC2 instance for video transcoding is less straightforward than AWS pricing charts suggest. Under burst conditions, a burstable \texttt{t3.micro} can be 29$\times$ more cost-efficient than a Compute-Optimized instance---yet na\"{i}ve horizontal scaling, where each node requires independent environment setup, inverts this advantage: provisioning overhead can consume up to 93.3\% of total execution time, eliminating any savings from parallelism.

This paper reports on a systematic empirical study covering 15 instance types (Burstable, General Purpose, and Compute-Optimized, including ARM Graviton), three codecs (H.264, H.265, VP9), and three scaling strategies (serial, horizontal, vertical), with $n=30$ runs per configuration. We identify provisioning overhead as the critical bottleneck, quantify its impact across the instance family spectrum (57.3\%--93.3\%), and demonstrate that eliminating per-instance environment setup via pre-built, workload-optimized AMIs resolves it---enabling a 10-node \texttt{t3.micro} cluster to outperform a single \texttt{m5.4xlarge} by $1.28\times$ while cutting base-compute cost by 89.4\% (62\% after CPU credit surcharges). This speedup pattern holds intra-family (10$\times$ \texttt{m5.large} vs.\ \texttt{m5.4xlarge}), confirming the benefit generalizes beyond burstable instances. A chunk-size sensitivity analysis further reveals that, once compute is optimized, data egress displaces it as the dominant cost driver. ARM Graviton results confirm competitive H.265 performance at lower cost. The findings yield statistically grounded guidelines for choosing between horizontal and vertical scaling, selecting instance families, and sizing clusters for cloud video transcoding.
\end{abstract}
...
Video transcoding is among the most compute-intensive operations in modern media delivery, and cloud platforms have become the default execution environment for it. For Video-as-a-Service (VaaS) operators---serving streaming, surveillance, and large-scale content distribution---the choice of EC2 instance family is not merely a technical decision; it directly shapes operating margins~\cite{b1,b17}.

Conventional wisdom often recommends Compute-Optimized instances for encoding-heavy workloads, assuming raw throughput directly translates to cost efficiency. Our measurements suggest a more nuanced picture: under burst conditions and the codecs and workload volumes examined here, Burstable instances can be up to 29$\times$ more cost-efficient despite their lower raw throughput. What prevents operators from realizing that advantage at scale is not a hardware limitation---it is a systems issue: instance provisioning overhead---the ``cold start'' time before encoding begins.

Prior work has explored general cloud benchmarking~\cite{b1}, cloud execution of HPC workloads~\cite{b3}, and multi-cloud scheduling~\cite{b20}, but cost optimization across diverse instance families for media-specific workloads has received less attention. The comparison between Burstable, General Purpose, and Compute-Optimized instances is especially nuanced: Burstable instances introduce CPU credit mechanics and different baseline costs that prior benchmarking studies have not systematically quantified under realistic video encoding scenarios. We therefore focus on H.264, H.265, and VP9---codecs that represent the predominant share of active commercial pipelines. Emerging standards such as VVC offer higher compression ratios, but their limited hardware acceleration support and licensing uncertainty in current EC2 generations place them outside the scope of predominant commercial pipelines.

We evaluate AWS T, M, and C instance families, covering both x86 and ARM Graviton architectures, under three scaling strategies (serial, horizontal, vertical). Four questions structure the investigation:
\begin{enumerate}
...
The central finding is that provisioning latency---not compute capacity---is the binding constraint for horizontal scaling of short-lived transcoding jobs. In unoptimized deployments, it consumed 57.3\%--93.3\% of total processing time across T-family configurations. A custom workload-optimized AMI eliminates most of this overhead, and the scaling economics shift accordingly: a 10-node \texttt{t3.micro} cluster outperforms a single \texttt{m5.4xlarge} by $1.28\times$ while cutting compute cost by 89.4\% (62\% after CPU credit surcharges). The same $1.28\times$ speedup holds intra-family (10$\times$ \texttt{m5.large} vs.\ \texttt{m5.4xlarge}), confirming the pattern is not an artifact of cross-family comparison. A chunk size sensitivity analysis and egress cost model complete the picture: once compute is optimized, data transfer becomes the dominant cost driver.
...
The literature on cloud resource optimization for media workloads can be broadly divided into two categories: general cloud benchmarking and provisioning, and video-encoding-specific studies. Neither has focused on what we examine here---how instance family selection and provisioning strategy interact under repetitive, divisible encoding jobs. While infrastructure studies treat workloads as largely monolithic tasks, and encoding studies optimize for throughput-per-dollar, the economics of short-lived, horizontally-scaled video chunks depend critically on provisioning latency---a hidden constant that neither community systematically quantifies across cloud instance families.

On the infrastructure side, prior work has benchmarked cloud instances across multiple workload classes~\cite{b1}, evaluated cloud versus supercomputer execution for HPC workloads~\cite{b3}, and proposed cost-driven provisioning for scientific computing~\cite{b2}. Resource management surveys~\cite{b4} and thread scalability studies~\cite{b23} complement these perspectives. Crucially, none of these works explicitly characterize the environment provisioning overhead that penalizes horizontally scaled, chunk-based VaaS deployments.

On the media encoding side, Moina-Rivera et al.~\cite{b22} provide a broad survey of cloud and edge transcoding architectures, while Katsenou et al.~\cite{b8} analyze energy-codec trade-offs on embedded devices. Jiang et al.~\cite{b20} propose a multi-cloud cost-aware scheduler for big-data pipelines, with limited coverage of video-specific workload properties. The closest antecedent to our parallelism approach is ExCamera~\cite{excamera} (Fouladi et al.), which achieved 60--300$\times$ speedups via fine-grained serverless (AWS Lambda) parallelism. While we share the underlying chunked-parallelism paradigm, our focus shifts to IaaS economics: we investigate the overhead cost of ephemeral compute in EC2, where VM and environment provisioning latency creates distinct bottlenecks not present in serverless models---and where na\"{i}ve parallelism can \emph{degrade} performance, inverting the assumption that more parallelism always reduces cost. AWS Elemental MediaConvert~\cite{awsmediaconvert} abstracts provisioning overhead entirely as a managed service, but imposes black-box fixed-pricing tiers that eliminate instance-level control and margin optimization.

Finally, while Spot instance pricing~\cite{b21,b19} offers cost reductions beyond what we report, preemption risk complicates deterministic latency guarantees for VaaS SLAs, placing such optimizations outside our reproducible on-demand baseline. Cloud performance variability is well-documented~\cite{b18,b10}, motivating our use of 30 trials per configuration; the chunked workload decomposition draws on fog-cloud~\cite{b12} and distributed scheduling~\cite{b13} literature.
...
The study proceeds in four sequential phases, each building on the previous one. Phase~1 characterizes the hardware environment; Phase~2 benchmarks raw encoding performance across the instance space; Phase~3 evaluates scaling strategies under realistic pipeline conditions; and Phase~4 quantifies economic sensitivity to chunk duration and egress. All experiments ran on AWS in \texttt{us-east-1} (N. Virginia); selected configurations were replicated in \texttt{sa-east-1} (São Paulo) to assess regional price effects.

\subsection{Phase 1: Validation of Processor Homogeneity}

AWS does not guarantee processor model uniformity within an instance type. To characterize this variability before benchmarking, we deployed 100 instances of each configuration (T, M, C families) across all six availability zones (a--f) in \texttt{us-east-1}, each running Ubuntu 22.04 LTS, and read \texttt{/proc/cpuinfo} on every instance.

Results confirmed meaningful heterogeneity, consistent with prior cloud studies~\cite{b2, b5, b6, b7}: \texttt{t3.micro} in zone~\texttt{a} was 98\% Xeon~8259CL / 2\% Xeon~8175M; \texttt{m5.large} in zone~\texttt{c} split 60\%/40\% between 8259CL and 8175M; \texttt{c5.large} in zone~\texttt{b} split 74\%/26\% between 8275CL and 8124M. ARM Graviton instances (\texttt{t4g}, \texttt{m7g}, \texttt{c7g}) were fully homogeneous (100\%).

To prevent CPU model from confounding Phase~3 results, provisioning scripts verify \texttt{/proc/cpuinfo} immediately after SSH becomes available. Any instance running a non-target processor is terminated and relaunched---to ensure that performance differences are attributable to instance type and not CPU model variation---and the loop repeats until the correct model is confirmed. For T-family (98\% dominant), this resolves on the first attempt in nearly all runs; M and C families (60--74\% dominant) required additional retries in a minority of cases. All 30 runs per configuration were on the same CPU model. Phase~3 ARM experiments use Graviton~3 only (\texttt{m7g.large}, \texttt{c7g.large}); \texttt{t4g} (Graviton~2) was validated but excluded from scaling tests due to its CPU credit mechanics.

\subsection{Phase 2: Initial Performance Benchmarks}

Phase~2 measures raw encoding performance across all 15 instance types using a containerized FFmpeg environment (H.264/AVC, H.265/HEVC, VP9).

The workload is a 320$\times$240 (240p) test sequence with mixed motion. This resolution is intentional: it ensures the encoding bottleneck remains in the CPU and codec thread scheduler rather than in I/O or memory bandwidth, isolating instance-level compute differences that would be masked at 1080p or 4K by memory and disk pressure on smaller instances. Segments are 30 seconds: long enough to make process initialization overhead (container startup, FFmpeg loading) negligible relative to encoding time, short enough to permit 30 replications per configuration without excessive experiment duration. Each condition ran 30 times. Encoding used:
...
Phase~3 applies the instance ranking from Phase~2 to a full pipeline question: for a 10-minute video, does horizontal scaling (ten parallel small instances) beat vertical scaling (one large instance)? A 10-minute workload is long enough to require chunked parallel processing but short enough to support repeated end-to-end trials.

Three configurations are compared across all T, M, and C families (x86 and ARM): horizontal (10$\times$ small instance), vertical (single large instance), and serial (1$\times$ small instance, full video). Regional pricing effects were assessed by replicating selected configurations in \texttt{sa-east-1}.

Two sub-phases structure the scaling investigation. Phase~3.1 (Dynamic Provisioning) uses standard Ubuntu images with post-boot FFmpeg installation, establishing the unoptimized overhead baseline. Phase~3.2 (Optimized Provisioning) uses a pre-configured custom AMI with FFmpeg pre-installed, measuring the ceiling achievable after eliminating runtime setup costs. Both phases record end-to-end wall-clock time and total cost.

\subsection{Phase 3 Extended Evaluation: ARM Graviton and Regional Pricing}

To assess architectural generality and geographic pricing sensitivity, Phase~3 was extended in two directions. First, two ARM Graviton 3 instances (\texttt{m7g.large} and \texttt{c7g.large}) were evaluated alongside their x86 counterparts using the same horizontal and serial scaling strategies. ARM experiments added H.265 (libx265) and VP9 (libvpx-vp9) as additional codecs to evaluate codec-specific performance differences across architectures. The same $n=30$ repetition protocol and terminate-and-retry CPU verification methodology described in Phase~1 were applied.
...
Execution time metrics were recorded with millisecond resolution. Each configuration was repeated $n=30$ times across both Phase~2 and Phase~3. Because $n=30$, the Central Limit Theorem ensures the sampling distribution of the mean approximates normality, justifying the use of Welch's t-test even when the underlying data deviate from normality. We computed descriptive statistics and 95\% Confidence Intervals (Student's t-distribution) for all configurations. Statistical analysis included Shapiro-Wilk tests for normality assessment, Levene's tests for variance homogeneity, and Welch's t-tests for group comparisons.
...
AWS does not guarantee processor model uniformity within an instance type, and our Phase~1 measurements confirmed meaningful heterogeneity, consistent with prior cloud benchmarking work~\cite{b5, b6}. T-family (\texttt{t3.micro}) instances in availability zone~\texttt{a} were 98\% Intel Xeon Platinum 8259CL / 2\% 8175M---effectively homogeneous. General Purpose (M) and Compute Optimized (C) families showed a 60\%/40\% split between processor models, substantial enough to confound latency comparisons if not controlled.

For Phase~3, we pinned execution to the dominant processor per family: 8259CL for M-type, 8275CL for C-type. Any instance that launched on a non-target processor was terminated immediately and re-launched; the loop continued until \texttt{/proc/cpuinfo} confirmed the target model. All 30 runs per configuration were on the same CPU model.

\subsection{Phase 2 Results: Initial Performance Benchmarks}

Phase~2 benchmarks H.264, H.265, and VP9 across all 15 instance types and multiple thread counts, establishing the baseline cost-performance landscape from which Phase~3 scaling decisions follow. Each configuration ran 30 times on isolated 30-second segments; results are reported as Compression Time (seconds), Efficiency (Eq.~\ref{eq:efficiency}), and Total Cost (Eq.~\ref{eq:total_cost}).

\subsubsection{Efficiency and Cost Calculation}

The two metrics are mathematically dual: $\text{Efficiency} = 1/\text{Total Cost}$, with $\text{cost}_{(h)}$ drawn from Table~\ref{tab:experimental_configurations} and $\text{time}_{(s)}$ the measured encoding duration. Total Cost gives an absolute monetary figure per operation; Efficiency gives a relative index that is more readable when comparing values that span orders of magnitude.
...
Table~\ref{tab:final_comparison_consistent} addresses \textbf{RQ1} directly. Three patterns stand out.

\textbf{Cost-efficiency vs.\ raw speed:} The \texttt{c5.4xlarge} is the fastest single instance (H.264, 10 threads: 1.1355s), but the most cost-efficient by a wide margin is the \texttt{t3.micro}---\$0.0000073 per operation vs.\ \$0.0002145 for the \texttt{c5.4xlarge}, a 29$\times$ difference (Efficiency 136,424 vs.\ 4,662). Paying for Compute-Optimized capacity buys speed, not value.

\textbf{Family hierarchy:} T-family leads on cost-efficiency across all codec and thread combinations; C-family leads on raw throughput at proportionally higher per-operation cost; M-family sits between the two, trading some compute performance for a more balanced cost profile.

\textbf{Thread scaling:} Every family shows diminishing returns beyond the instance's vCPU count, consistent with thread oversubscription effects~\cite{b1}. For the single-vCPU \texttt{t2.micro}, this ceiling is immediate; for \texttt{c5.4xlarge} (16 vCPUs), gains continue up to the thread limit before the curve flattens.
...
slower but $\approx$6.9$\times$ more cost-efficient ($p < .001$, $d = 19.1$). While conventionally extreme, these large Cohen's $d$ values are a direct consequence of the minimal measurement variance inherent to containerized FFmpeg workloads (coefficient of variation $< 0.6\%$): in this highly deterministic regime, even marginal absolute performance differences yield statistically massive effect sizes. Under these near-normal
...
location estimator, which better characterizes central tendency in the presence of skewed distributions.

These visualizations reveal a pattern of diminishing returns in thread scaling: moving
from 1 to 3 threads yields near-linear time reductions across smaller instances
(e.g., \texttt{t3.micro}), but gains plateau at 5 threads for smaller instances.
...
The plateau in smaller instances and the regression in VP9
...
Phase~2 identified \texttt{t3.micro} as the most cost-efficient configuration for H.264 encoding in isolation. Phase~3 asks whether that efficiency survives at scale---specifically, when a 10-minute video must be encoded end-to-end using horizontal (10-node cluster) versus vertical (single large instance) deployment. A key measurement scope difference: Phase~2 measured raw encoding time on isolated 30-second segments (median $\approx$2.5\,s per segment on \texttt{t3.micro}); Phase~3 measures complete pipeline wall-clock time including provisioning, file transfer, encoding, and output retrieval. The higher Phase~3 values ($\approx$73\,s per cluster run) are not slower encoding---they include $\approx$48\,s of provisioning overhead that is invisible to Phase~2.

H.264 across all x86 families is used as the primary scaling benchmark to isolate provisioning dynamics from codec-specific confounds (Table~\ref{tab:scaling_all_families}). H.265 and VP9 are evaluated specifically for ARM Graviton comparisons, where codec sensitivity across architectures is the question of interest.

Within the x86 evaluation, each 10-node cluster (10$\times$ the smallest instance in a family) is compared against the largest single instance in that family: \texttt{t3.2xlarge}, \texttt{m5.4xlarge}, and \texttt{c5.4xlarge}. The nearest cost-equivalent vertical baseline for the \texttt{t3.micro} cluster (\$0.104/h) is \texttt{t3.xlarge} (\$0.1664/h), but we benchmark against the largest nodes (up to \$0.768/h) to test whether small-instance clusters can outperform---not just match---the highest-throughput option within each tier. ARM Graviton tests were restricted to M and C families; T-family Graviton variants (\texttt{t4g}) were excluded because their credit mechanics introduce confounds in sustained workloads.

\subsubsection{Two-Phase Analysis Results}

Phase~3.1 used standard Ubuntu images with post-boot FFmpeg installation---the simplest possible deployment---to establish an unoptimized baseline for \textbf{RQ2}. The results were stark: setup time consumed 57.3--93.3\% of total wall-clock time depending on configuration. Even so, vertical scaling (\texttt{t3.2xlarge}, 3.04 min) edged out the horizontal cluster (\texttt{t3.micro}$\times$10, 3.28 min) by 7.3\%. What the horizontal cluster gained was on cost: \$0.00569 versus \$0.01686 for vertical---a 66.3\% reduction despite the performance gap. The takeaway was clear: provisioning overhead is the obstacle. Removing it is the goal of Phase~3.2.

Phase~3.2 replaced runtime dependency installation with pre-configured AMIs, addressing \textbf{RQ3}. The effect was immediate and consistent. The optimized \texttt{t3.micro} cluster completed in 1.22 minutes---a $1.48\times$ speedup over the vertical \texttt{t3.2xlarge} (1.80 min). Across the larger families, the pattern held: M-family horizontal reached 1.22 min ($1.28\times$ over \texttt{m5.4xlarge} at 1.56 min); C-family reached 1.18 min ($1.26\times$ over \texttt{c5.4xlarge} at 1.49 min). The serial \texttt{t3.micro} baseline was 2.72 min. In every family, the horizontal cluster was both faster and cheaper than the largest available vertical instance once provisioning overhead was removed.

\subsubsection{Analysis and Validation}

In parallel configurations, \textit{Effective Encoding Time} includes raw FFmpeg computation plus SFTP segment upload and output retrieval; \textit{Setup Overhead} (synonymous with provisioning latency throughout this paper) is the time from EC2 \texttt{run\_instances} API call until the SSH daemon is reachable. All experimental artifacts are available at \url{https://anonymous.4open.science/r/ec2sweetspot-blinded2}.

Dynamic Phase~3.1 data for 10$\times$\texttt{m5.large} is absent from Table~\ref{tab:scaling_all_families}: all 30 runs exhibited systematic FFmpeg muxer overflow on segments~1 and~4, consistent with a known defect in the FFmpeg version distributed via Ubuntu~20.04 \texttt{apt}. The defect does not appear in Phase~3.2, which uses a pre-built AMI with a newer FFmpeg build.

Table~\ref{tab:scaling_all_families} reports full timing and cost data across both phases; Figure~\ref{fig:unified_scaling} visualizes how the performance gap between horizontal and vertical scaling collapses when AMI optimization removes the provisioning bottleneck.
...
The Phase~3.2 results are consistent across all three families: once provisioning overhead is removed, horizontal clusters are faster \emph{and} cheaper than the largest vertical instance in every case. T-family achieved the strongest relative speedup: $1.48\times$ (1.22 min vs.\ 1.80 min for \texttt{t3.2xlarge}). M-family: $1.28\times$ (1.22 min vs.\ 1.56 min for \texttt{m5.4xlarge}); C-family: $1.26\times$ (1.18 min vs.\ 1.49 min for \texttt{c5.4xlarge}).

Within-family cost savings are modest in absolute terms: M-family horizontal cut cost by 2.3\% (\$0.01952 vs.\ \$0.01997); C-family by 1.0\% (\$0.01672 vs.\ \$0.01689). These margins are expected---per-hour rates for smaller instances within a family scale roughly proportionally to those of larger ones, so the main within-family benefit of horizontal scaling is throughput, not cost.

The cost advantage becomes pronounced in cross-family comparisons. Each \texttt{t3.micro} node handles a 60-second chunk in a single burst well within the credit window, delivering 100\% vCPU throughput during encoding---comparable to non-burstable instances for this workload duration. Under this regime, the \texttt{t3.micro} cluster (\$0.00211) represents an 89.4\% cost reduction over \texttt{m5.4xlarge} (\$0.01997) and 87.5\% over \texttt{c5.4xlarge} (\$0.01689), while being faster than both.

The C-family intra-family result is notable on its own: ten \texttt{c5.large} nodes beat one \texttt{c5.4xlarge} in both speed and cost. Horizontal scaling is not a burstable-instance trick; it is a general property of divisible workloads once provisioning is controlled.

Dynamic Phase~3.1 data for 10$\times$\texttt{m5.large} is excluded from Table~\ref{tab:scaling_all_families}: all 30 runs exhibited systematic FFmpeg muxer overflow (``too many packets buffered'') on segments~1 and~4, consistent with a known defect in the FFmpeg version distributed via Ubuntu~20.04 \texttt{apt}. No complete encoding succeeded. The defect does not appear in Phase~3.2, which uses a pre-built AMI with a patched FFmpeg build---further illustrating the practical value of AMI optimization beyond provisioning speed.
...
Welch's t-tests confirm the Phase~3 speedup advantages are not artifacts of variance: $p < 10^{-23}$ for T-family horizontal vs.\ vertical ($d = 3.74$); $p < 10^{-44}$ for M-family ($d = 8.77$) and C-family ($d = 7.69$). These are large, robust effects. ARM Graviton encoding variance is low when isolated from transfer artifacts: all reconstructed configurations in Table~\ref{tab:arm_graviton_scaling} achieve CV$<$10\%, with provisioning itself measured at CV=1.9\% ($n=100$)---more stable than the corresponding x86 measurement (CV=4.4\%).

The intra-family comparisons---10$\times$\texttt{m5.large} vs.\ \texttt{m5.4xlarge} and 10$\times$\texttt{c5.large} vs.\ \texttt{c5.4xlarge}---serve as the methodologically cleanest test of the horizontal scaling hypothesis. Both sides sit within the same instance family and billing tier, eliminating confounds from CPU credit mechanics, cross-family pricing gaps, and heterogeneous workload behavior. The $1.28\times$ and $1.26\times$ speedups observed in M and C families under these controlled conditions confirm that the advantage is architectural, not an artifact of comparing burstable and non-burstable instances.

The T-family cross-family result (10$\times$\texttt{t3.micro} vs.\ \texttt{m5.4xlarge} or \texttt{c5.4xlarge}) amplifies this: the cost savings (89.4\% and 87.5\%) are substantial, though part of that figure reflects the pricing gap between burstable and general-purpose families. The intra-family result establishes the floor; the cross-family result shows the ceiling available to operators willing to mix instance classes.
...
ARM Graviton and regional pricing were evaluated as independent extensions to the Phase~3 framework, each targeting a specific generalizability question: whether the x86 findings transfer to a different ISA, and whether they hold outside \texttt{us-east-1}.

\textbf{ARM Graviton Performance (Codecs):} Testing \texttt{c7g.large} and \texttt{m7g.large} alongside their x86 counterparts revealed architecture-codec interactions. For H.264 horizontal encoding, x86 was faster: \texttt{c5.large} reached 64.04s median vs.\ 107.04s for \texttt{m7g.large}. ARM Graviton's advantage surfaced under H.265: both the \texttt{m7g.large} (59.27s median) and \texttt{c7g.large} (61.03s median) horizontal clusters reached comparable H.265 performance, with no statistically significant difference between them ($p = .47$). Comparing these ARM H.265 numbers to x86 H.264 numbers would be misleading---the codecs differ in computational complexity, so such a comparison conflates encoder workload with architecture.

ARM did not benefit from horizontal scaling for H.264. The serial \texttt{m7g.large} (57.55s) outperformed its horizontal cluster (107.04s), because the $\approx$6s per-chunk H.264 encoding time on Graviton is small enough that the $\approx$48s provisioning overhead swamps any parallelism gain. This is the same provisioning bottleneck seen in Phase~3.1 x86 data, but amplified by faster per-chunk encoding.

The high H.265 serial times ($\approx$700s for both \texttt{m7g.large} and \texttt{c7g.large}) reflect the computational intensity of \texttt{libx265} \texttt{-preset slow} on ARM: current \texttt{libx265} builds lack the SIMD optimization depth available on x86, making H.265 encoding disproportionately slower on Graviton relative to H.264. For context, the same 10-minute H.265 encoding on x86 (\texttt{c5.large} serial) completes in $\approx$72s---an order of magnitude faster---highlighting the architecture--codec interaction.

Table~\ref{tab:arm_graviton_scaling} reports Phase~3.2 results for all ARM configurations. Two H.265 serial rows carry an adjusted $n$: during the execution of \texttt{m7g.large} and \texttt{c7g.large}, isolated container orchestration anomalies caused complete pipeline stalls in a small number of runs, producing extreme outliers ($>$5 standard deviations from the mean). These corrupted runs were excluded to preserve the integrity of the central tendency analysis, yielding $n=26$ (\texttt{m7g.large}) and $n=29$ (\texttt{c7g.large}). The resulting \texttt{m7g.large} H.265 serial distribution is unimodal with low variance (median 700.51s, stdev 4.10s).
...
\textbf{Statistical Validation:} Welch's t-test found no significant difference between \texttt{m7g.large} and \texttt{c7g.large} for H.265 horizontal encoding ($p > .05$), with moderate variability (CV 4.5--6.2\%) in H.265 horizontal runs. A Mann-Whitney U test confirmed the same conclusion ($p > .05$), providing a non-parametric corroboration. Welch's t-test confirmed the H.264 inversion on \texttt{m7g.large} at high confidence ($p < 10^{-12}$, Cohen's $d = 4.31$): this is not statistical noise but a structural effect of provisioning overhead dominating encoding time at the chunk granularity used.

To isolate cloud-side variance from local experimental artifacts (SFTP transfer contention), Table~\ref{tab:arm_graviton_scaling} reports reconstructed times: measured encoding duration plus the median provisioning latency from an independent $n=100$ characterization experiment (20.5\,s, CV=1.9\% for ARM Graviton---more stable than x86 at CV=4.4\%). Under this decomposition, all ARM configurations achieve CV$<$10\%.

\textbf{Regional Cost Disparities:} Replication in \texttt{sa-east-1} (São Paulo) shows a 54--62\% price premium over \texttt{us-east-1}, varying by family (T: +61.5\%, M: +59.4\%, C: +54.1\%). For H.264 horizontal deployments (10$\times$ nodes, 60s chunks), cluster costs roughly double: \texttt{t3.micro} \$0.0021 vs.\ \$0.0045; \texttt{m5.large} \$0.0150 vs.\ \$0.0339; \texttt{c5.large} \$0.0152 vs.\ \$0.0234. Wall-clock times are consistent across regions, confirming that compute performance is hardware-bound rather than region-bound---the cost difference is entirely a pricing artifact.

\subsection{Phase 4 Results: Chunk Size Sensitivity and Cost Analysis}
\label{results-phase4}

Phase~4 complements the scaling results with two analyses that address \textbf{RQ4}: an empirical sensitivity study on chunk duration, and a cost model for network egress.

\textbf{Chunk Size Sensitivity:}
The total wall-clock time for a horizontally scaled cluster depends on chunk duration $c$ through a non-monotone relationship. Let the cluster process a video of length $L$ with $N = \lceil L/c \rceil$ instances in parallel:
\begin{equation}
T(c) = t_{\text{seg}} + \max_{1 \le i \le N} \left( t_{\text{prov},i} \right) + t_{\text{enc}}(c) + t_{\text{merge}}
\end{equation}
Reducing $c$ increases $N$ and raises the expected maximum provisioning time (the synchronization barrier at the end of the parallel phase). Increasing $c$ lowers $N$ but reduces parallelism, making encoding time the bottleneck. We measured this trade-off empirically at $c \in \{30, 60, 120\}$s on the 10-minute H.264 workload with the Phase~3.2 AMI across all three x86 families. Total Time in Table~\ref{tab:chunk_sensitivity} is end-to-end wall-clock; speedup is relative to the single-instance vertical baseline of each family (\texttt{t3.2xlarge}: 1.81\,min; \texttt{m5.4xlarge}: 1.56\,min; \texttt{c5.4xlarge}: 1.49\,min).

For 240p content, the 60-second configuration was optimal in every family: 30-second chunks degraded performance because 20 provisioning sequences widen the long-tail outlier; 120-second chunks reduced the cluster to only 5 workers, yielding too little parallelism. At 60s, speedup over the vertical baseline reached 1.48$\times$ (T), 1.28$\times$ (M), and 1.26$\times$ (C).

\textbf{Resolution Dependence:} To test whether this optimum generalizes beyond low-resolution workloads, we replicated the M-family chunk sensitivity experiment at 720p and 1080p using \texttt{m5.large} horizontal clusters (Table~\ref{tab:chunk_multiresolution}). The optimal chunk size shifts with resolution: at 240p, where per-chunk encoding ($\approx$5\,s) is dwarfed by provisioning ($\approx$18\,s), 60-second chunks balance parallelism against provisioning overhead. At 720p and 1080p, encoding time grows by $10\times$ and $30\times$ respectively, making parallelism the dominant factor---30-second chunks become optimal because the marginal cost of additional provisioning is amortized over substantially longer per-chunk encoding. Transfer overhead via SFTP also scales with chunk file size, contributing 26--199\,s per chunk at higher resolutions; an S3-based pipeline would reduce this component but was not evaluated.
...
\begin{table}[h]
	...
	\resizebox{\columnwidth}{!}{
		\begin{tabular}{|l|c|c|c|c|}
			...
			\hline
			\multirow{3}{*}{1080p (1920$\times$1080)} & \textbf{30s} & \textbf{6.77} & 156.6 & 26.3 \\
			& 60s & 9.35 & 236.6 & 138.3 \\
			& 120s & 13.04 & 399.5 & 198.6 \\
			\hline
		\end{tabular}
	}
\end{table}
...
A CDN-fronted delivery path (S3\,+\,CloudFront) reduces this cost by $\approx$10$\times$: CloudFront charges \$0.0085/GB vs.\ \$0.09/GB for direct egress, making CDN delivery cheaper than compute above $\approx$2\,GB/month of output.
...
The results support optimized horizontal scaling as a cost reduction strategy. The following paragraphs address the main constraints and threats to generalizability.

\textbf{AMI vs.\ runtime-install overhead:} In Phase~3.2, FFmpeg and all dependencies are pre-baked into the custom AMI; no package installation occurs at runtime. The residual provisioning overhead ($\approx$48\,s) reflects EC2 launch, EBS volume attachment, OS boot, and SSH readiness only. In Phase~3.1, FFmpeg is installed post-boot via \texttt{apt}, which accounts for the substantially higher setup times in that phase. These two phases are not interchangeable baselines.

\textbf{Burstable CPU credit surcharges:} T-family instances run in Unlimited mode~\cite{awscredits} and draw Surplus Credits immediately on launch. Because the orchestration pattern here is strictly ephemeral (spawn, encode, terminate), instances never accumulate idle time to offset that draw. We bound the surcharge exposure rather than reporting a single figure: under a realistic encoding-focused assumption ($\approx$22\,s at full CPU), the per-cluster surcharge is $\approx$\$0.0055, bringing the cluster cost to $\approx$\$0.0076---62\% below \texttt{m5.4xlarge}. In the worst case (100\% CPU for the full 73\,s wall time), the surcharge reaches $\approx$\$0.0183 and eliminates the cost advantage entirely. Billing statements were not audited; the true value falls somewhere between these bounds depending on actual CPU utilization during non-encoding phases.

\textbf{Alternative baselines:} We evaluate on-demand EC2 with custom orchestration. AWS-native services (Elemental MediaConvert, Fargate) achieve near-zero provisioning overhead by design; spot instances~\cite{b21,b19} offer lower costs at the price of preemption risk. Both comparisons are left for future work.

\textbf{Transfer overhead:} SFTP was used for chunk distribution within a single AZ. Because horizontal configurations transfer $N$ chunks simultaneously from the orchestrating host while vertical transfers a single file, SFTP systematically penalizes horizontal scaling relative to S3-based pipelines. The reported speedup advantages are therefore conservative lower bounds.
...
For short-form video encoding on EC2, the dominant cost driver is not the CPU but the time spent waiting for instances to become ready. Provisioning overhead consumed up to 93.3\% of wall-clock time in unoptimized horizontal deployments, negating burstable-instance cost advantages entirely. Eliminating that overhead via workload-optimized AMIs enabled a 10-node \texttt{t3.micro} cluster to outperform a single \texttt{m5.4xlarge} by $1.28\times$ while cutting base-compute cost by 89.4\%---a result that held intra-family as well. ARM Graviton clusters achieved competitive H.265 horizontal performance (median 59.27\,s, CV$<$7\%). Multi-resolution replication revealed that the optimal chunk size is resolution-dependent: 60\,s for 240p but 30\,s for 720p and 1080p, where encoding time dominates provisioning. Once compute is optimized, outbound egress becomes the binding cost constraint. Provisioning overhead mitigation is the prerequisite for cost-effective horizontal cloud transcoding; GPU-accelerated instances and spot pricing are the most promising next directions.
