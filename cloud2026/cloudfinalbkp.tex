...
\title{The EC2 Sweet Spot: Cost Optimization and Scaling Strategies for Video-as-a-Service Workloads}
...
\begin{IEEEkeywords}
Cloud Computing; Resource Allocation; Cost Optimization; Video Transcoding;
Horizontal Scaling; Vertical Scaling; Instance Provisioning; Workload
Scheduling; Amazon Web Services; Distributed Systems; XaaS; Video-as-a-Service
\end{IEEEkeywords}
...
specialization~\cite{b1}, examined encoding performance without detailed cost
analysis~\cite{b3}, or addressed multi-cloud scheduling without considering the
...
This study addresses these limitations through a comprehensive empirical analysis of AWS Burstable (T), General Purpose (M), and Compute Optimized (C) instance families---encompassing both traditional x86 and modern ARM Graviton architectures---under video transcoding workloads. Four primary research questions guide this investigation:
\begin{enumerate}
...
Our primary contribution is the rigorous quantification and architectural mitigation of cloud provisioning overhead for episodic workloads. In unoptimized configurations, provisioning latency accounted for 57.3\%--93.3\% of total processing time across T-family configurations. By developing a custom Amazon Machine Image (AMI) framework, this bottleneck was mitigated, altering the scaling economics: a distributed cluster of 10$\times$ \texttt{t3.micro} instances outperformed single-node vertical deployments of the largest evaluated x86 instances, achieving up to a $1.28\times$ speedup and an 89.4\% compute cost reduction (cross-family: \texttt{t3.micro} cluster vs.\ \texttt{m5.4xlarge}). Notably, the same $1.28\times$ speedup is also observed intra-family (10$\times$ \texttt{m5.large} vs.\ \texttt{m5.4xlarge}), indicating that horizontal scaling advantages hold within---not only across---instance families. Furthermore, we extend the state-of-the-art by proposing both an empirical chunk size sensitivity analysis and a network egress cost formulation. These models indicate that once compute efficiency is maximized through horizontal orchestration and architectural matching (e.g., ARM Graviton for specific codecs), data egress becomes a dominant economic constraint. Ultimately, these findings support a dynamic, stateless approach to distributed media processing that maximizes both computational throughput and financial efficiency.

This paper is structured as follows: Section~\ref{rel_works} reviews related
work in cloud resource optimization and video processing. Section~\ref{met}
details the experimental methodology across four phases.
Section~\ref{experiments} presents the corresponding results.
Section~\ref{limitations} discusses threats to internal, external, and
construct validity. Section~\ref{conclusion} summarizes contributions and
outlines future research directions. Supplementary experimental artifacts are
...
Video processing resource optimization poses research challenges due to modern
codec complexity. Advanced standards like Versatile Video Coding (VVC)
introduce sophisticated encoding tools that increase computational demands,
driving research into algorithmic optimizations~\cite{b15, b16}. While prior
work, such as that by Dancheva et al.~\cite{b3}, focuses on
comparative encoding performance, our investigation shifts focus to the cloud
infrastructure itself, analyzing its performance and cost-effectiveness for
practical deployment.

Cloud resource optimization has been studied from multiple perspectives. Prior
work includes cost-driven provisioning methodologies for scientific computing,
as demonstrated by Álvarez et al.~\cite{b2}, and surveys categorizing
resource management strategies~\cite{b4}. Our research extends this foundation
by demonstrating that workload-optimized horizontal architectures can
simultaneously outperform vertical scaling for video encoding workloads in both
processing speed and cost-efficiency.

Recent work by Jiang et al.~\cite{b20} introduces PIVOT, a
cloud-agnostic framework that addresses cost-aware scheduling in multi-cloud
environments. Their approach tackles significant challenges in cross-cloud data
analysis, including infrastructure heterogeneity and prohibitive egress costs.
While PIVOT focuses on multi-cloud workflow orchestration for big data
applications, our work complements this by providing a fine-grained analysis of
cost-performance trade-offs across Burstable, General Purpose, and Compute
Optimized instance families, specifically for video encoding workloads.

Recent research explores the nuances of video encoding deployments on cloud
infrastructure. Moina-Rivera et al.~\cite{b22} provided a review of
cloud and edge media encoding and transcoding techniques, highlighting
architectural challenges and emphasizing the economic benefits of adaptive,
pay-as-you-go cloud models. While their review establishes a theoretical
foundation for media encoding architectures, our study extends this work by
contributing an empirical case study evaluating cost-performance trade-offs
across Burstable, General Purpose, and Compute Optimized instances.

Zabrovskiy et al.~\cite{b21} introduced FSpot, an intelligent
scheduling heuristic explicitly targeting Amazon EC2 spot instances to minimize
video encoding costs. Their approach reduces expenses by rapidly benchmarking
and dynamically estimating encoding speeds for lower-cost but preemptible
computing units. Similarly, Ben-Yehuda et al.~\cite{b19} deconstructed Amazon EC2 spot instance pricing dynamics to understand cost minimization opportunities. While these
strategies provide significant economic incentives by leveraging unpredictable
spot markets (typically 70--90\% cheaper than On-Demand), they introduce
preemption risks that may force workload restarts. Our methodology establishes
a deterministic alternative: carefully orchestrated horizontal scaling across
stable instance families can achieve comparable cost-efficiency while
circumventing the risks of sudden instance termination, guaranteeing completion
without interruption.

Cloud optimization problems exhibit a well-documented multi-objective
nature~\cite{b8,b14,b1}. Studies such as Katsenou et al.~\cite{b8}
provide detailed analyses of energy efficiency trade-offs across different
codecs and hardware platforms. Furthermore, Maas et al.~\cite{b1}
evaluated cost-performance trade-offs for parallel applications across 104 HPC
and general-purpose instances, revealing that tailoring instance selection to
specific workload characteristics yields measurable improvements. Our work
aligns with this approach but introduces a focus on video encoding.

In a related line of inquiry, Maas et al.~\cite{b23} examined how
thread scalability characteristics influence performance and cost optimization
for parallel workloads across three major cloud providers. They demonstrated
that matching an application's thread-level parallelism to the appropriate
instance type prevents unnecessary expenditure on redundant cores. While their
analysis focuses on general HPC applications, our research extends these
scalability principles to divisible video encoding workloads across Burstable,
General Purpose, and Compute Optimized instance families to minimize
operational overhead.

Benchmarking methodology is a recurring concern in cloud performance
research. Comparative studies illustrate the heterogeneity in cloud
technologies, highlighting challenges in establishing direct performance
comparisons~\cite{b18, b1}. Our methodology addresses this challenge through
environmental standardization, using containerized execution environments that
align with the reproducible frameworks proposed by Ferreira et al.~\cite{b9}.

Our research addresses cloud performance variability through empirical
measurement. We quantify variability across 30 trials per configuration,
establishing baseline performance profiles for cost-efficiency analysis. This
data-driven approach complements predictive modeling techniques proposed in
prior work~\cite{b10, b11}.

The architectural principles underlying our parallel processing case study
build upon established workload decomposition strategies. The approach of
segmenting large, divisible tasks to improve cost-efficiency has been
demonstrated in contexts such as task scheduling in fog-cloud
environments~\cite{b12, b13}. Our contribution lies in adapting these
principles specifically for parallel video encoding within public cloud
infrastructure, demonstrating how managing provisioning overhead enables
deterministic performance gains.

Fouladi et al.~\cite{excamera} introduced ExCamera, a system that encodes video in parallel using thousands of short-lived cloud functions (AWS Lambda), decomposing a video into fine-grained segments processed concurrently and merged via a coordinator. ExCamera demonstrated that aggressive task parallelism can reduce end-to-end encoding latency by orders of magnitude compared to sequential pipelines. Our work shares the chunked-parallelism principle but focuses on EC2 instance economics rather than serverless latency minimization, empirically quantifying the provisioning overhead trade-offs that serverless architectures implicitly absorb.

Managed transcoding services such as AWS Elemental MediaConvert~\cite{awsmediaconvert} offer near-zero provisioning overhead by design, making them relevant alternatives for VaaS deployments. However, they operate as black-box managed services with fixed pricing tiers, offering no visibility into instance-level cost-performance trade-offs. Our work complements this landscape by quantifying the cost-performance dynamics of self-managed EC2 deployments, where orchestration overhead is tunable.

The diminishing returns observed in thread and instance scaling are consistent with Amdahl's Law~\cite{b1}, which bounds parallel speedup by the serial fraction of a workload. Chunk-based video encoding can reduce the serial fraction to near zero (segmentation and merge steps), but provisioning latency introduces a new non-parallelizable bottleneck that Amdahl's classical formulation does not capture. This motivates our empirical analysis of provisioning overhead as the primary scalability constraint.

Table~\ref{tab:comparative_analysis} summarizes the positioning of our work
relative to related research, highlighting the gaps addressed by each study.
Our work provides a unified empirical analysis of cost-efficiency trade-offs
across Burstable, General Purpose, and Compute Optimized instance families in
parallel video encoding architectures, with particular emphasis on operational
overhead quantification and practical AMI-based optimization strategies.
...
experimental phases, conducted on AWS infrastructure in the us-east-1 region.
...
and 8124M variants (Table~\ref{tab:processors}).

\begin{table}[H]
	\centering
	\caption{Processor distribution across availability zones: representative
	examples per instance family (100 instances per zone). Full distribution
	across zones a--f available in the artifact repository.}
	\small
	\label{tab:processors}
	\begin{tabular}{|c|c|c|c|}
		\hline
		\textbf{Family} & \textbf{A. Zone} & \textbf{Architecture} & \textbf{Count} \\ \hline
		\multirow{2}{*}{T (\texttt{t3.micro})} & a & Xeon 8259CL 2.50GHz & 98 \\ \cline{2-4}
		& a & Xeon(R) 8175M @ 2.50GHz & 2 \\ \hline
		\multirow{2}{*}{M (\texttt{m5.large})} & c & Xeon 8259CL 2.50GHz & 60 \\ \cline{2-4}
		& c & Xeon(R) 8175M @ 2.50GHz & 40 \\ \hline
		\multirow{2}{*}{C (\texttt{c5.large})} & b & Xeon  8275CL 3.00GHz & 74 \\ \cline{2-4}
		& b & Xeon(R) 8124M @ 3.00GHz & 26 \\ \hline
	\end{tabular}
\end{table}
...
The experimental workload utilized 1080p standard test sequences representing
...
Docker container startup and FFmpeg loading are included in the measured wall-clock times; the 30-second segment duration was chosen so that this initialization overhead ($<$1\,s, as the container image is pre-installed on the AMI rather than pulled at runtime) remains negligible relative to the encoding phase.
...
and \texttt{libvpx-vp9}. The \texttt{<n>} parameter was tested with 1, 3, 5,
and 10 threads. Encoding duration was recorded and cost calculated based on
instance pricing.
...
To assess architectural generality and geographic pricing sensitivity, Phase~3 was extended in two directions. First, two ARM Graviton 3 instances (\texttt{m7g.large} and \texttt{c7g.large}) were evaluated alongside their x86 counterparts using the same horizontal and serial scaling strategies. ARM experiments added H.265 (libx265) and VP9 (libvpx-vp9) as additional codecs, since the ARM NEON SIMD instruction set may interact differently with codec-specific encoder implementations than x86 AVX2/SSE4.2 paths. The same $n=30$ repetition protocol and terminate-and-retry CPU verification methodology described in Phase~3 were applied. Following the discovery of log-parsing artifacts in preliminary ARM data (where codec attribution had been misread for a subset of runs), we re-parsed all 240 ARM experimental logs using a corrected consolidation script that validates codec attribution via filename pattern matching; no additional misattributions were found. Second, the horizontal scaling evaluation was replicated in the \texttt{sa-east-1} (São Paulo) region for three representative instances (\texttt{t3.micro}, \texttt{m5.large}, \texttt{c5.large}) using the H.264 codec to quantify the impact of regional pricing on the cost-performance trade-off identified in the primary \texttt{us-east-1} experiments.

\subsection{Phase 4: Chunk Size Sensitivity and Cost Analysis}

To extend the empirical findings, we developed two analyses. First, an Empirical Chunk Size Sensitivity Analysis characterizes how segment duration $c$ affects total wall-clock time $T(c)$ for a video of length $L$ encoded across $N = \lceil L/c \rceil$ parallel instances. The pipeline execution time decomposes into four observable components: segmentation ($t_{\text{seg}}$), provisioning and boot latency of the slowest node ($\max_i t_{\text{prov},i}$), per-chunk encoding ($t_{\text{enc}}(c)$), and output merge ($t_{\text{merge}}$). Three chunk durations were evaluated: 30\,s ($N=20$), 60\,s ($N=10$), and 120\,s ($N=5$). Second, a Network Cost Model evaluates the economic impact of data egress across the VaaS pipeline. Total pipeline cost $C_{\text{total}}$ is decomposed into compute provisioning cost $C_{\text{comp}} = \sum_{i=1}^{N} R_{\text{inst}} \times T_i$ (where $R_{\text{inst}}$ is the per-second billing rate and $T_i$ is the active lifespan of node $i$) and outbound transfer cost $C_{\text{net}} = R_{\text{egress}} \times D_{\text{out}}$ (where $R_{\text{egress}}$ is the regional egress rate and $D_{\text{out}}$ is the output payload in GB). These models are fitted against empirical measurements in Section~\ref{results-phase4}.

Execution time metrics were recorded with millisecond resolution. For Phase~2
benchmarks, each configuration was repeated $n=30$ times. With the observed low coefficient of variation for the majority of Phase~2 configurations (Table~\ref{tab:final_comparison_consistent}),
a two-sample Welch's t-test targeting 80\% power at $\alpha=0.05$ to detect a
5\% effect requires fewer than 10 observations; $n=30$ exceeds this threshold
and ensures sufficient power for detecting even subtle differences. We computed descriptive
...
to ensure statistical parity with Phase~2 results. To preserve data integrity
when evaluating provisioning latency, failed initialization attempts (such as
API rate limits or SSH timeouts) were discarded, and replacement executions
were instantiated until exactly 30 successful, complete end-to-end runs were
acquired for each cluster architecture. Across both experimental phases,
...
comparisons. Where Shapiro-Wilk rejected normality, Welch's t-test is retained
when justified either by the Central Limit Theorem (n=30 with low coefficient
of variation) or by effect sizes so extreme that parametric and non-parametric
tests yield identical conclusions; cases where neither condition holds are noted
explicitly as limitations.
...
research~\cite{b5, b6}. As detailed in Table~\ref{tab:processors}, processor
architecture varied within the same instance type and availability zone.
...
processors. General Purpose (M) and Compute Optimized (C) x86 instances exhibited a more pronounced architectural
...
Notably, the ARM architecture eliminated this processor heterogeneity concern. AWS Graviton 3 instances (\texttt{m7g.large}, \texttt{c7g.large})~\cite{awsgrav3} are powered by proprietary custom silicon exclusively designed and deployed by Amazon. This guarantees 100\% baseline homogeneity across all hardware nodes globally, eliminating the need for processor-pinning orchestration and establishing ARM as a uniquely stable baseline for horizontally scaled scientific computations.
...
Two metrics were employed for cost-performance characterization: Efficiency
(Equation~\ref{eq:efficiency}) and Total Cost (Equation~\ref{eq:total_cost}).
These metrics are mathematically dual: since Total Cost $= (\text{hourly rate}/3600) \times \text{time}$,
Efficiency reduces to $1/\text{Total Cost}$. They therefore encode the same
information and will always produce identical instance rankings. Both are
reported for interpretability: Total Cost provides an absolute monetary
figure in USD, while Efficiency yields a relative index that facilitates visual comparison across
orders of magnitude without requiring the reader to invert dollar values.
...
The cost variable corresponds to hourly instance rates from
Table~\ref{tab:experimental_configurations}, while time represents compression
duration in seconds. Mathematically, our Efficiency definition functions as an
\textit{Inverse Cost-Delay Product}, conceptually analogous to the
Energy-Delay Product used in hardware architecture. This non-linear metric
($1 / (\text{Cost} \times \text{Time})$) explicitly penalizes configurations
that are either excessively slow or disproportionately expensive, as an
increase in either execution delay or monetary cost drives the denominator up,
thereby reducing the efficiency score. Unlike single-variable metrics such as
raw ``speedup'' (which ignores budget) or ``work per dollar'' (which ignores
execution latency), the Inverse Cost-Delay Product addresses the
multi-objective optimization requirements of cloud management by rewarding
configurations that simultaneously minimize infrastructure expenditure and
execution delay. Total Cost (Equation~\ref{eq:total_cost}) complements this by
providing the actual monetary cost per operation.

\begin{table}[!htbp]
	\centering
	\caption{Experimental instance configurations and on-demand pricing (Linux, shared tenancy). us-east-1 prices retrieved March 2026 via AWS Pricing API. sa-east-1 prices for \texttt{t3.micro}, \texttt{m5.large}, \texttt{c5.large} reflect actual experimental billing; other sa-east-1 prices derived using the empirically observed 43.5\% regional premium.}
	...
	\begin{tabular}{|l|c|c|c|c|}
		...
		\hline
		\texttt{t2.micro}   & 1  & 1  & 0.0116 & 0.0166 \\
		\texttt{t3.micro}   & 2  & 1  & 0.0104 & 0.0149 \\
		\texttt{t3.small}   & 2  & 2  & 0.0208 & 0.0298 \\
		\texttt{t3.medium}  & 2  & 4  & 0.0416 & 0.0597 \\
		\texttt{t3.large}   & 2  & 8  & 0.0832 & 0.1194 \\
		\texttt{t3.xlarge}  & 4  & 16 & 0.1664 & 0.2387 \\
		\texttt{t3.2xlarge} & 8  & 32 & 0.3328 & 0.4776 \\
		\hline
		\multicolumn{5}{|l|}{\textit{General Purpose (M5-family, x86)}} \\
		\hline
		\texttt{m5.large}   & 2  & 8  & 0.0960 & 0.1380 \\
		\texttt{m5.xlarge}  & 4  & 16 & 0.1920 & 0.2754 \\
		\texttt{m5.2xlarge} & 8  & 32 & 0.3840 & 0.5510 \\
		\texttt{m5.4xlarge} & 16 & 64 & 0.7680 & 1.1021 \\
		\hline
		\multicolumn{5}{|l|}{\textit{Compute Optimized (C5-family, x86)}} \\
		\hline
		\texttt{c5.large}   & 2  & 4  & 0.0850 & 0.1220 \\
		\texttt{c5.xlarge}  & 4  & 8  & 0.1700 & 0.2440 \\
		\texttt{c5.2xlarge} & 8  & 16 & 0.3400 & 0.4879 \\
		\texttt{c5.4xlarge} & 16 & 32 & 0.6800 & 0.9758 \\
		\hline
		\multicolumn{5}{|l|}{\textit{ARM Graviton3 (M7g/C7g-family, aarch64)}} \\
		\hline
		\texttt{m7g.large}  & 2  & 8  & 0.0816 & 0.1171 \\
		\texttt{c7g.large}  & 2  & 4  & 0.0725 & 0.1040 \\
		\hline
	\end{tabular}
\end{table}
...
infrastructure scale alone does not strictly dictate processing speed.
...
\begin{table*}[h]
	...
	\resizebox{\textwidth}{!}{
		\begin{tabular}{ll|rrrrr|rrrrr|rrrrr}
			...
			m5.large & 3 thread & 2.4412 & 0.0110 & 2.4324 & 2.4807 & 15,361 | 0.0000651 & 4.1942 & 0.0302 & 4.1660 & 4.2747 & 8,940 | 0.0001118 & 17.1400 & 0.0636 & 17.0100 & 17.2800 & 2,188 | 0.0004570 \\
			...
			m5.large & 10 thread & 2.4795 & 0.0294 & 2.4382 & 2.5349 & 15,124 | 0.0000661 & 4.1685 & 0.0203 & 4.1418 & 4.2330 & 8,996 | 0.0001112 & 17.1800 & 0.0915 & 17.1000 & 17.4300 & 2,182 | 0.0004582 \\
			\midrule
			...
			m5.xlarge & 5 thread & 1.6210 & 0.0130 & 1.5909 & 1.6458 & 11,567 | 0.0000865 & 2.4587 & 0.0103 & 2.4425 & 2.4801 & 7,626 | 0.0001311 & 16.8100 & 0.0211 & 16.7700 & 16.8600 & 1,115 | 0.0008963 \\
			m5.xlarge & 10 thread & 1.6307 & 0.0138 & 1.6042 & 1.6581 & 11,498 | 0.0000870 & 2.4056 & 0.0094 & 2.3854 & 2.4266 & 7,794 | 0.0001283 & 16.8200 & 0.0336 & 16.7700 & 16.9100 & 1,114 | 0.0008973 \\
			\midrule
			m5.2xlarge & 1 thread & 2.5233 & 0.0139 & 2.5117 & 2.5671 & 3,715 | 0.0002692 & 2.6472 & 0.0176 & 2.6051 & 2.6796 & 3,541 | 0.0002824 & 16.8000 & 0.0472 & 16.7500 & 16.9300 & 557 | 0.0017924 \\
			...
			m5.2xlarge & 10 thread & 1.3986 & 0.0242 & 1.3219 & 1.4256 & 6,702 | 0.0001492 & 1.9016 & 0.0111 & 1.8886 & 1.9360 & 4,930 | 0.0002028 & 16.8100 & 0.0427 & 16.7500 & 16.9500 & 557 | 0.0017932 \\
			\midrule
			...
			m5.4xlarge & 5 thread & 1.2975 & 0.0394 & 1.2675 & 1.4102 & 3,612 | 0.0002768 & 1.8671 & 0.0132 & 1.8486 & 1.9036 & 2,510 | 0.0003983 & 16.8400 & 0.0376 & 16.7900 & 16.9200 & 278 | 0.0035915 \\
			m5.4xlarge & 10 thread & 1.3023 & 0.0291 & 1.2841 & 1.4223 & 3,599 | 0.0002778 & 1.7620 & 0.0162 & 1.7373 & 1.8092 & 2,660 | 0.0003759 & 16.8900 & 0.0496 & 16.7900 & 16.9800 & 277 | 0.0036026 \\
			\midrule\midrule
			...
			c5.large & 3 thread & 2.1765 & 0.0037 & 2.1710 & 2.1878 & 19,459 | 0.0000514 & 3.2957 & 0.0747 & 3.2602 & 3.4680 & 12,850 | 0.0000778 & 15.0400 & 0.0184 & 15.0100 & 15.0800 & 2,816 | 0.0003551 \\
			c5.large & 5 thread & 2.1709 & 0.0026 & 2.1655 & 2.1762 & 19,509 | 0.0000513 & 3.6417 & 0.0094 & 3.6283 & 3.6766 & 11,630 | 0.0000860 & 15.0200 & 0.0218 & 14.9800 & 15.0600 & 2,820 | 0.0003545 \\
			...
			\midrule
			c5.xlarge & 1 thread & 2.2561 & 0.0096 & 2.2445 & 2.2935 & 9,386 | 0.0001065 & 2.6163 & 0.0118 & 2.5963 & 2.6560 & 8,093 | 0.0001235 & 14.8700 & 0.0327 & 14.8000 & 14.9400 & 1,424 | 0.0007020 \\
			...
			c5.xlarge & 5 thread & 1.4498 & 0.0134 & 1.4184 & 1.4718 & 14,606 | 0.0000685 & 2.1616 & 0.0090 & 2.1448 & 2.1769 & 9,796 | 0.0001021 & 14.8800 & 0.0382 & 14.8000 & 14.9500 & 1,423 | 0.0007027 \\
			c5.xlarge & 10 thread & 1.4567 & 0.0145 & 1.4218 & 1.4829 & 14,537 | 0.0000688 & 2.1164 & 0.0095 & 2.1004 & 2.1393 & 10,005 | 0.0000999 & 14.8700 & 0.0387 & 14.8100 & 14.9500 & 1,423 | 0.0007024 \\
			\midrule
			c5.2xlarge & 1 thread & 2.3049 & 0.0151 & 2.2927 & 2.3448 & 4,593 | 0.0002177 & 2.3651 & 0.0219 & 2.3203 & 2.4164 & 4,476 | 0.0002234 & 15.0900 & 0.0453 & 15.0000 & 15.1800 & 701 | 0.0014250 \\
			...
			c5.2xlarge & 10 thread & 1.2549 & 0.0256 & 1.2028 & 1.3032 & 8,437 | 0.0001185 & 1.6833 & 0.0120 & 1.6651 & 1.7058 & 6,290 | 0.0001590 & 15.1200 & 0.0415 & 15.0600 & 15.1900 & 700 | 0.0014283 \\
			\midrule
			c5.4xlarge & 1 thread & 2.2658 & 0.0108 & 2.2528 & 2.3082 & 2,336 | 0.0004280 & 2.1498 & 0.0207 & 2.1176 & 2.2090 & 2,462 | 0.0004061 & 14.4500 & 0.0390 & 14.3800 & 14.5500 & 366 | 0.0027295 \\
			...
			c5.4xlarge & 10 thread & 1.1355 & 0.0259 & 1.1250 & 1.2371 & 4,662 | 0.0002145 & 1.5050 & 0.0128 & 1.4740 & 1.5290 & 3,517 | 0.0002843 & 14.5000 & 0.1485 & 14.4000 & 14.9600 & 365 | 0.0027395 \\
			\bottomrule
		\end{tabular}
	}
\end{table*}
...
location estimator, which is more robust to the rare outliers observed in H.265
distributions. These visualizations
...
(e.g., \texttt{t3.micro}), the gains flatten sharply at 5 threads and often
regress at 10 threads. This performance plateau is a direct consequence of
thread oversubscription: as the number of active software threads exceeds the
available physical cores, the resulting synchronization overhead---including
context switching, shared memory contention, and inter-thread
coordination---overtakes the computational gains of additional parallelism.
...
\texttt{c5.4xlarge}), though scaling benefits plateau beyond 10 threads or when
oversubscription occurs. The VP9 codec (\texttt{libvpx-vp9}) exhibits a
distinct behavior, maintaining near-constant execution times across all thread
configurations \textit{within} a given instance type. Notably, this
thread-agnostic pattern does not imply uniformity across instance types:
VP9 execution times span a $\approx$25\% range across the evaluated families (e.g., \texttt{c5.4xlarge} at 14.45\,s vs.\ \texttt{t3.large} at 18.06\,s, a 25.0\% difference),
reflecting underlying CPU throughput differences. The invariance is with
respect to thread count---without explicit structural parameters such as
spatial tiling (which is outside the scope of this study and was not
configured in any experiment), the encoder cannot effectively exploit additional vCPU
resources under default \texttt{libvpx-vp9} settings, rendering parallelized vertical scaling inefficient for VP9
under the evaluated configuration.

\begin{figure}[!ht]*
	\centering
	\includegraphics[width=0.4\textwidth]{graphs/unified_codecs_bar_chart.png}
	\caption{Compression time (mean $\pm$ 95\% CI, $n=30$) across all instance
	...
	diminishing returns for H.264 and H.265, while VP9 exhibits rigid
	thread-agnostic behavior across all configurations.}
...
cost-optimal configuration for H.264 compression, we present a comprehensive scaling
...
file transfer, encoding, and output retrieval. Scaling the Phase~2 per-second
encoding rate to 60-second chunks yields an expected encoding time of $\approx$5\,s
per node---consistent with the $\approx$6\,s per-chunk encoding observed in
Phase~3 logs. The substantially larger Phase~3 wall-clock values ($\approx$73\,s per
cluster run) reflect the addition of provisioning overhead ($\approx$48\,s), FFmpeg
process startup, and SFTP transfer, which are excluded from Phase~2 measurements. Our Phase~3 evaluation establishes its baseline by isolating the traditional H.264 codec across all x86 architectural families, isolating provisioning dynamics from codec-specific confounds (Table~\ref{tab:scaling_all_families}). Subsequently, rather than exhaustively re-tabulating the entire x86 grid, we selectively deploy the denser H.265 (HEVC) and VP9 modern encoders specifically to benchmark the custom ARM Graviton 3 hardware (\texttt{c7g}, \texttt{m7g}) against established x86 performance baselines, revealing codec-specific performance differences across architectures. For the primary H.264 scaling evaluation, our methodology contrasts a cluster of ten
...
The \textit{Effective Encoding Time} metric warrants clarification in the
parallel context. For a single \texttt{t3.micro} processing the 10-minute
video serially (Phase~3.2), effective encoding takes 1.72 minutes (103
seconds), implying a theoretical per-instance computational share of
$\approx 0.17$ minutes when distributed across 10 nodes. The reported parallel
Effective Encoding Time of 0.22 minutes reflects both raw FFmpeg computation
and inherent network I/O transfers (SFTP segment upload and output retrieval).
Orchestration is implemented as a lightweight, stateless Python controller that
pre-segments the source video using FFmpeg, synchronously provisions target
EC2 instances via the AWS API (\texttt{boto3}), and establishes parallel
SSH/SFTP tunnels to dispatch segments, execute encoding via the custom AMI, and
retrieve compressed outputs. Accordingly, Effective Encoding Time encapsulates
raw FFmpeg computation plus intrinsic network I/O transfers, while
\textit{Setup Overhead} bounds the API-to-Running provisioning latency from
instance creation request until SSH daemon accessibility. Orchestration code,
custom AMI build configurations, and complete experimental logs are publicly
available in the artifact repository (\url{https://anonymous.4open.science/r/ec2sweetspot-blinded}) for
reproducibility.
...
\begin{table*}[!htbp]
\centering
\caption{Comparison of scaling strategies across optimization phases for Burstable (T), General Purpose (M), and Compute Optimized (C) families. Time values include $\pm$3\% derived 95\% CI margin of error. \textbf{Speedup} is relative to the serial baseline of the same family and provisioning phase. \textbf{Cost Factor} is total cost relative to the serial AMI (Phase~3.2) baseline of the same family. ARM Graviton results are presented separately in Table~\ref{tab:arm_graviton_scaling}, as ARM experiments used only the optimized AMI (Phase~3.2) without a dynamic provisioning baseline.}
\label{tab:scaling_all_families}
\resizebox{\textwidth}{!}{%
\scriptsize
\begin{tabular}{lll*{5}{c}}
\toprule
\textbf{Family} & \textbf{Strategy} & \textbf{Provisioning} & \textbf{Total Time (min)} & \textbf{Setup Overhead (\%)} & \textbf{Total Cost (USD)} & \textbf{Speedup} & \textbf{Cost Factor} \\
\midrule
\multicolumn{3}{c}{\textbf{Burstable (T)}} \\
& Serial   & Ubuntu & 4.03 $\pm$ 0.12 & 57.3\% & \$0.00070 & 1.00$\times$ & 1.0$\times$ \\
& Serial   & AMI    & 2.72 $\pm$ 0.08 & 36.8\% & \$0.00047 & 1.00$\times$ & 1.0$\times$ \\
& Parallel & Ubuntu & 3.28 $\pm$ 0.10 & 93.3\% & \$0.00569 & 1.23$\times$ & 8.1$\times$ \\
& Parallel & AMI    & 1.22 $\pm$ 0.04 & 82.0\% & \$0.00211 & 2.23$\times$ & 4.5$\times$ \\
& Vertical & Ubuntu & 3.04 $\pm$ 0.09 & 74.0\% & \$0.01686 & 1.33$\times$ & 24.1$\times$ \\
& Vertical & AMI    & 1.80 $\pm$ 0.05 & 56.1\% & \$0.00998 & 1.51$\times$ & 21.2$\times$ \\
\midrule
\multicolumn{3}{c}{\textbf{General Purpose (M)}} \\
& Serial   & Ubuntu & 3.46 $\pm$ 0.10 & 50.6\% & \$0.00554 & 1.00$\times$ & 1.0$\times$ \\
& Serial   & AMI    & 2.71 $\pm$ 0.08 & 36.9\% & \$0.00434 & 1.00$\times$ & 1.0$\times$ \\
& Parallel & Ubuntu & 3.63 $\pm$ 0.11 & 92.8\% & \$0.05808 & 0.95$\times$ & 10.5$\times$ \\
& Parallel & AMI    & 1.22 $\pm$ 0.04 & 78.7\% & \$0.01952 & 2.22$\times$ & 4.5$\times$ \\
& Vertical & Ubuntu & 2.33 $\pm$ 0.07 & 74.7\% & \$0.02982 & 1.48$\times$ & 5.4$\times$ \\
& Vertical & AMI    & 1.56 $\pm$ 0.05 & 62.2\% & \$0.01997 & 1.74$\times$ & 4.6$\times$ \\
\midrule
\multicolumn{3}{c}{\textbf{Compute Optimized (C)}} \\
& Serial   & Ubuntu & 3.21 $\pm$ 0.10 & 53.9\% & \$0.00455 & 1.00$\times$ & 1.0$\times$ \\
& Serial   & AMI    & 2.46 $\pm$ 0.07 & 39.8\% & \$0.00349 & 1.00$\times$ & 1.0$\times$ \\
& Parallel & Ubuntu & 3.52 $\pm$ 0.11 & 93.2\% & \$0.04987 & 0.91$\times$ & 11.0$\times$ \\
& Parallel & AMI    & 1.18 $\pm$ 0.04 & 79.7\% & \$0.01672 & 2.08$\times$ & 4.8$\times$ \\
& Vertical & Ubuntu & 2.13 $\pm$ 0.06 & 75.6\% & \$0.02414 & 1.51$\times$ & 5.3$\times$ \\
& Vertical & AMI    & 1.49 $\pm$ 0.04 & 65.1\% & \$0.01689 & 1.65$\times$ & 4.8$\times$ \\
\bottomrule
\end{tabular}%
}
\end{table*}
...
For cross-family comparisons, the contrast is far more pronounced. These comparisons pair burstable T-family instances with general-purpose M-family and compute-optimized C-family instances, which differ in CPU burst behavior and SLA guarantees. The comparison is justified for the specific episodic, short-duration workloads studied here: each \texttt{t3.micro} node processes a 60-second chunk in an ephemeral burst that completes before CPU credits are exhausted---within the burst window, burstable instances deliver sustained 100\% vCPU performance comparable to non-burstable alternatives. Under this burst-compatible workload regime, the \texttt{t3.micro} cluster (\$0.00211) achieved an 89.4\% cost reduction
...
statistical framework used in Phase~2. Shapiro-Wilk tests confirmed non-normal
distributions for Phase~3 execution times across all configurations ($p <
.05$). Levene's test revealed no significant difference in variances between
horizontal and vertical architectures for the M and C families ($p > .05$), but
indicated unequal variances for the T-family ($p = .034$). Welch's t-tests
confirmed statistically significant speedup advantages for horizontal scaling
across all families ($p < 10^{-23}$ for T-family; $p < 10^{-44}$ for M and C
families). Unlike Phase~2, Phase~3 coefficient of variation is not uniformly
low; however, the observed $p$-values are so extreme that any rank-based
non-parametric alternative (e.g., Mann-Whitney~U) would yield an identical
conclusion. Effect sizes were quantified using Cohen's $d$, yielding large
values across all x86 families ($d = 3.74$ for T-family, $d = 8.77$ for
M-family, and $d = 7.69$ for C-family). Note that Cohen's conventional benchmarks (small: 0.2, medium: 0.5, large: 0.8) are not meaningful at this scale; values of this magnitude ($d > 3$) do not imply a practically larger effect than, say, $d = 1.5$---they primarily reflect the near-deterministic nature of containerized FFmpeg measurements (CV~$< 0.6\%$), which produces artificially large $d$ values. Rather, these values
indicate effectively complete distributional separation between the compared
groups---a condition under which non-parametric alternatives (e.g., Cliff's
$\delta$) asymptote to their maximum value of $\pm 1$. Consequently, the
normality assumption underlying Cohen's $d$ is inconsequential for the
qualitative conclusion: the performance separation between horizontal and
vertical architectures is not attributable to random variation regardless of
the effect size metric chosen. ARM Graviton configurations, which exhibit substantially
higher variance (CV~$>50\%$ for H.265), are treated as preliminary results
and their statistical interpretation is discussed as a threat to validity in
...
\textbf{ARM Graviton Performance (Codecs):} The introduction of ARM Graviton instances (\texttt{c7g.large}, \texttt{m7g.large}) alongside their x86 counterparts revealed critical architectural sensitivities. While x86 instances exhibited superior performance for traditional H.264 encoding under horizontal scaling (e.g., \texttt{c5.large} achieved a median time of 64.04s in \texttt{us-east-1}), ARM Graviton demonstrated codec-specific parallel encoding advantages for H.265 (HEVC)---though this advantage coexists with high operational variance (CV~$>50\%$), as detailed below.
...
For the H.264 codec on ARM, serial execution (57.55\,s) outperformed the parallel cluster architecture (108.07\,s)---an inversion consistent with provisioning overhead dominating the brief encoding phase. With a fixed 30\,s SSH wait plus $\approx$18\,s EC2 boot latency per node, the parallel cluster spends approximately 48\,s in setup before encoding begins, while a 60\,s H.264 chunk encodes in only $\approx$6\,s on \texttt{m7g.large}---meaning provisioning overhead accounts for $\approx$89\% of the combined provisioning-plus-encoding time (48\,s out of $\approx$54\,s), and dominates the encoding phase by a factor of $8\times$. The full 108\,s wall-clock time incorporates additional uninstrumented phases beyond this breakdown. Causal attribution to provisioning (rather than other factors such as SFTP transfer or synchronization jitter) is supported by this quantitative breakdown but cannot be established without direct instrumentation isolating each phase.
...
The central finding---that unoptimized \texttt{libx265} on ARM Graviton encodes at near-real-time speed ($\approx$700\,s for a 10-minute video)---is robust and unaffected by the artifact removal. This pronounced slowdown suggests a software-hardware encoding performance gap: without explicit NEON SIMD assembly optimizations compiled into the default Ubuntu \texttt{libx265} binaries~\cite{neonvideo}, the encoder operates in a degraded execution mode on the Graviton architecture. These results suggest that migrating encoding pipelines to ARM may require custom-compiled, architecture-aware binaries to achieve optimal performance, a hypothesis warranting direct empirical validation.

\begin{table*}[h]
	\centering
	\caption{Descriptive statistics for compression time (seconds), efficiency, and total cost for ARM Graviton configurations. $^{\dagger}$H.265 serial statistics corrected after removing log-parsing artifact: m7g.large n=26 (1 misattributed VP9 run excluded); c7g.large n=29 (1 misattributed H.264 run excluded). All other rows use n=30.}
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{ll|rrrrr|rrrrr|rrrrr}
			...
			m7g.large & horizontal & 108.0700 & 16.6122 & 60.9900 & 150.0600 & 3 | 0.0024496 & \textbf{59.2650} & 30.5831 & 58.0900 & 225.7900 & \textbf{12} | \textbf{0.0013433} & 105.2050 & 27.3938 & 59.0200 & 134.8600 & 3 | 0.0023846 \\
			\cmidrule(lr){1-17}
...
\textbf{Statistical Validation of Novel Architectures:} To rigorously confirm the performance inversions observed on Graviton instances, we applied the same statistical framework utilized in Phase~2. Tests of normality (Shapiro-Wilk) confirmed that execution times for ARM configurations were non-normally distributed ($p < .001$). For H.265 horizontal encoding, comparing the \texttt{m7g.large} against \texttt{c7g.large} revealed equal variances via Levene's test ($p = .33$). Given the high CV ($>50\%$) for H.265 horizontal data---indicating strong non-normality---we rely on the non-parametric Mann-Whitney~U test as the primary test of difference, which yielded a non-significant result ($p > .05$). Cohen's $d$ is not reported for this comparison, as its assumptions are violated at CV~$>50\%$. The available evidence does not support a statistically significant performance difference between the M and C families for H.265 on Graviton; however, the high variance (CV~$>50\%$) limits statistical power, and formal equivalence testing (e.g., TOST) was not performed.

The large standard deviation (30.58s) and maximum (225.79s) for the \texttt{m7g.large} horizontal H.265 strategy (Table~\ref{tab:arm_graviton_scaling}) warrant attention. These long-tail outliers are artifacts of non-deterministic cloud provisioning latency (e.g., API throttling, EBS mounting delays, and IP assignment jitter upon launching 10 concurrent nodes). While the median (59.27s) is robust to such variability, the high coefficient of variation ($>50\%$) reflects the operational variability of parallelized cloud architectures and limits the reliability of this configuration for latency-sensitive deployments.

Conversely, comparing the highly unusual H.264 performance inversion on \texttt{m7g.large} (where Serial outperformed Horizontal), Levene's test indicated unequal variances ($p < .01$). Welch's t-test confirmed the statistical significance of this inversion ($p < 10^{-12}$, Cohen's $d = 4.31$). This finding strongly suggests that non-deterministic cloud orchestration constraints are a primary determinant of horizontal scaling viability: the provisioning-to-encoding ratio of $8\times$ quantified above renders the strategy inviable regardless of the underlying CPU architecture. Because our horizontal strategy initiates 10 ephemeral cloud instances concurrently and acts as a synchronous barrier, the workflow's total wall-clock time is bounded by the single slowest initializing node. For \texttt{libx264} on Graviton, the estimated 48\,s provisioning floor per node ($\approx$30\,s SSH readiness wait plus $\approx$18\,s boot latency) substantially exceeds the $\approx$6\,s per-chunk encoding time, yielding a provisioning-to-encoding ratio of $8\times$. It should be noted that 48\,s + 6\,s = 54\,s accounts for only two identified components, whereas the observed median of 108\,s for \texttt{m7g.large} H.264 horizontal includes additional uninstrumented overhead---most plausibly SFTP file transfers and SSH session teardown for result retrieval ($\approx$50--55\,s combined). The 48\,s floor is therefore a lower bound on total overhead, not a complete timing decomposition; direct per-phase instrumentation would be required to attribute the remaining gap. Consequently, the overhead of managing 10 elastic nodes renders horizontal scaling substantially disadvantageous compared to a single serial pipeline under these conditions.

\textbf{Regional Cost Disparities:} We replicated the horizontal scaling evaluation in the \texttt{sa-east-1} (São Paulo) region to establish constraints on geographic deployment. Empirical billing data confirmed an approximate 44\% premium over \texttt{us-east-1} (N. Virginia) across all utilized instance types (hourly rates in Table~\ref{tab:experimental_configurations}). For the H.264 Horizontal strategy (10$\times$ nodes, 60\,s chunks), median end-to-end wall-clock time and total cluster cost per run were, respectively: \texttt{t3.micro}: 73.20\,s / \$0.0021 (us-east-1) vs.\ 96.64\,s / \$0.0040 (sa-east-1); \texttt{c5.large}: 64.04\,s / \$0.0152 vs.\ 64.41\,s / \$0.0218; \texttt{m5.large}: 56.41\,s / \$0.0150 vs.\ 79.85\,s / \$0.0306.

While the optimized horizontal orchestration still strictly outperformed vertical scaling within the \texttt{sa-east-1} boundary, the cross-regional threshold indicates that compute optimization must be weighed against data residency laws. Furthermore, executing structurally identical horizontal workloads across different geopolitical AWS zones exposes regional capacity volatility: as demonstrated by the $\sim$23-second performance inflation for \texttt{m5.large} horizontal in São Paulo compared to N. Virginia, regions equipped with smaller baseline infrastructure pools exhibit significantly higher API throttling and hypervisor allocation latency when synchronously provisioning 10 parallel instances. Therefore, transferring workloads to cheaper regions may be offset by unpredictable orchestration \textit{jitter} or cross-region data egress costs.
...
The findings reveal a clear sensitivity boundary: 30-second chunks across 20 nodes increase parallel provisioning coordination proportionally (20 bootstrap sequences instead of 10), which, for shorter encoding tasks, erodes the speedup advantage. Meanwhile, 120-second chunks with only 5 nodes reduce provisioning calls but fail to exploit parallelism sufficiently, as the higher per-instance encoding burden dominates and produces sub-optimal acceleration. Notably, the T-family 120-second result (2.04\,min) is higher than a na\"{\i}ve arithmetic estimate based on linear chunk-time scaling ($\approx$1.44\,min) would suggest. Log analysis suggests this gap arises from two compounding factors: (1)~SFTP segment transfer time is hypothesized to scale with chunk size, plausibly adding $\approx$30\,s for 2$\times$ larger files, though transfer and encoding phases were not instrumented independently and this attribution cannot be established without direct per-phase measurement; and (2)~\texttt{t3.micro} CPU burst credits were partially depleted during this measurement, causing per-chunk encoding to take $\approx$43\,s rather than the $\approx$22\,s observed under full-credit conditions---a direct consequence of the burstable instance model and a threat to the reproducibility of this specific configuration. The 60-second configuration achieves the optimal balance between provisioning coordination cost and parallelism exploitation, yielding the highest speedup across all three families (1.48$\times$ for T, 1.28$\times$ for M, and 1.26$\times$ for C). This inverted-U boundary underscores the need for dynamic chunk sizing algorithms in production VaaS pipelines.

\begin{table}[h]
	\centering
	\caption{Chunk Size Sensitivity Analysis (10-min H.264, Phase 3.2 AMI).
	Total Time is end-to-end wall-clock. Speedup vs.\ each family's vertical baseline.}
...
While optimized chunking minimizes compute expenditure, full cloud pipeline economics demand a deterministic cost model. Total Cost ($C_{\text{total}}$) is formulated as the sum of compute provisioning ($C_{\text{comp}}$) and outbound data transfer ($C_{\text{net}}$):
\begin{equation}
C_{\text{total}} = \left( \sum_{i=1}^{N} R_{\text{inst}} \times T_{i} \right) + \left( R_{\text{egress}} \times D_{\text{out}} \right)
\end{equation}
where $R_{\text{inst}}$ is the per-second instance billing rate, $T_i$ is the active billing lifespan of node $i$,\footnote{AWS imposes a 60-second minimum billing floor per instance: any $T_i < 60$\,s is billed as 60\,s. All $T_i$ values observed in our experiments exceed this threshold.} $R_{\text{egress}}$ is the regional outbound internet data transfer rate (e.g., \$0.09/GB in \texttt{us-east-1}, \$0.15/GB in \texttt{sa-east-1}), and $D_{\text{out}}$ is the final payload size in GB.

Our network analysis of an optimized VaaS pipeline (10-minute 1080p source, 500\,MB input, 150\,MB H.264 output) indicates that intra-region component transfers (Amazon EBS and EC2 $\leftrightarrow$ S3) accumulate negligible operational costs ($\sim$\$0.01 per video). However, outbound internet egress imposes a strict, non-optimizable economic floor. For example, delivering the 150\,MB encoded payload incurs $\approx$\$0.0135 in \texttt{us-east-1} (150\,MB $\div$ 1{,}000\,MB/GB $\times$ \$0.09/GB, using AWS decimal billing units). When contrasted against our heavily optimized horizontal \texttt{t3.micro} cluster compute cost of \$0.00211, network egress costs are approximately $6.4\times$ more expensive than the underlying encoding computation itself. The egress-compute break-even point---where egress cost equals the optimized compute cost---occurs at an output payload of approximately 23.4\,MB (i.e., \$0.00211 $\div$ \$0.09/GB $\approx$ 0.0234\,GB). For reference output files from a 10-minute 1080p H.264 encode ($\approx$150\,MB), egress dominates by a factor of 6$\times$. Thus, once compute provisioning logic is optimized, data transfer emerges as the primary, inescapable long-term cost for VaaS platforms---a constraint that cannot be addressed through additional compute optimization. For repeated delivery of the same encoded asset, Amazon CloudFront reduces egress cost to $\approx$\$0.0085/GB, a $10.6\times$ reduction over direct egress (\$0.09/GB), yielding a per-delivery cost of $\approx$\$0.00128 for the same 150\,MB output---below even the optimized compute cost of \$0.00211. CDN pre-positioning is therefore economically justified for any output payload in our workload range, and direct-egress pricing should be treated as a strict upper bound on delivery cost rather than an operational target.
...
While our findings advocate for optimized horizontal scaling, several
operational constraints and validity threats must be acknowledged when
generalizing these results.

\subsection{Internal Validity}

\textbf{Docker Container vs.\ AMI Overhead:} It is important to distinguish between two sources of initialization overhead in the experimental design. In Phase~3.2 (Optimized Provisioning), the custom AMI pre-installs the Docker image with FFmpeg and all dependencies; no Docker layer pull from a registry occurs at runtime. The observed provisioning overhead ($\approx$48\,s) therefore reflects EC2 instance launch time, EBS volume attachment, OS boot, and SSH readiness wait---not Docker registry download. In Phase~3.1 (Dynamic Provisioning), FFmpeg dependencies are installed post-boot via \texttt{apt}, which includes package download time from Ubuntu repositories but not Docker image pull (FFmpeg is installed natively, not containerized, in Phase~3.1 for simplicity). This distinction is critical for correctly attributing the setup overhead differences between the two sub-phases.

\textbf{Burstable Instance Sustainability and CPU Credits:} T-family instances
operate in Unlimited mode by default, allowing immediate 100\% CPU bursts but
relying on a credit accrual system~\cite{awscredits}. A newly launched
\texttt{t3.micro} begins with a zero credit balance; intensive workloads
therefore immediately consume Surplus Credits---a mechanism allowing
performance bursts prior to earning credits. In ephemeral orchestration
patterns (spawn, encode, terminate), instances are deliberately terminated
before accumulating idle time to repay these credit deficits. AWS
automatically converts outstanding surplus credits into billing surcharges upon
instance termination. However, because the H.264 workloads complete in
approximately 1 minute per micro-instance, the accumulated surplus credits
are expected to translate to negligible terminal surcharges. Two bounds characterize the credit surcharge exposure. Under the absolute worst case (100\% CPU for the full 73\,s wall-clock duration), a \texttt{t3.micro} accumulates 2\,vCPUs $\times$ 90\% surplus fraction $\times$ (73\,s $\div$ 3600) $\approx$ 0.03650 vCPU-hours of surplus credits at \$0.05/vCPU-hour, yielding a surcharge of $\approx$\$0.00183 per instance, or $\approx$\$0.0183 for 10 instances. Added to the base compute cost of \$0.00211, this raises the cluster cost to $\approx$\$0.0204---marginally exceeding the \texttt{m5.4xlarge} (\$0.01997) and eliminating the cost advantage under this extreme assumption. Under a realistic encoding-focused estimate (sustained CPU load only during the $\approx$22\,s encoding phase, with provisioning and SFTP transfer at low utilization), the per-instance surcharge falls to $\approx$\$0.00055, or $\approx$\$0.0055 for 10 instances, raising the cluster cost to $\approx$\$0.0076---a $\approx$62\% reduction versus the \texttt{m5.4xlarge}. \textit{Actual AWS billing statements were not audited to confirm the precise surcharge applied in practice; the true value lies between these bounds depending on workload CPU intensity.}

\subsection{Construct Validity}

\textbf{T-Family Cost Metric Accuracy:} The 89.4\% compute cost reduction (\$0.00211 vs.\ \$0.01997) is based on instance billing rates without auditing actual AWS billing statements. As detailed above, the CPU credit surcharge analysis yields two bounds: under a realistic encoding-focused assumption ($\approx$22\,s at full CPU), the surcharge is $\approx$\$0.0055 for 10 instances, raising the cluster cost to $\approx$\$0.0076 and preserving a $\approx$62\% reduction; under the absolute worst case (100\% CPU for 73\,s), the surcharge is $\approx$\$0.0183, raising the cluster cost to $\approx$\$0.0204---marginally exceeding the \texttt{m5.4xlarge} baseline and eliminating the cost advantage entirely. The 89.4\% base-compute figure and the $\approx$62\% encoding-adjusted figure represent the most and least favorable plausible scenarios under realistic workload assumptions; the absolute worst case should be treated as a limiting boundary rather than an expected outcome. Verification of actual AWS billing would resolve this uncertainty.

\textbf{Billing Granularity Assumptions:} The cost model assumes AWS per-second billing (in effect since October 2017), subject to a 60-second minimum floor per instance. This floor systematically benefits the horizontal architecture: a \texttt{t3.micro} node active for $\approx$73\,s is billed for 73\,s rather than being penalized, whereas a return to per-minute billing would round up to 120\,s---nearly doubling the effective billed time and substantially narrowing the horizontal cost advantage. The model should be re-evaluated if AWS modifies its billing granularity.

\textbf{Serverless and Managed Service Alternatives:} This study evaluates on-demand EC2 instances with custom orchestration. AWS-native managed services such as AWS Elemental MediaConvert and AWS Fargate offer near-zero provisioning overhead by design, potentially eliminating the bottleneck quantified in this work. A direct comparison against these alternatives would strengthen the positioning of the AMI-based horizontal scaling approach and is an important direction for future work.

\textbf{Spot Instance Comparison:} The related work (Zabrovskiy et al., Ben-Yehuda et al.) documents spot instances as 70--90\% cheaper than on-demand. This study uses on-demand pricing throughout to ensure deterministic completion without preemption risk. The comparison between on-demand horizontal scaling and spot-priced alternatives is not evaluated empirically; spot instances represent an important baseline for future cost comparisons.

\subsection{External Validity}

\textbf{Network Conditions and Transfer Overhead:} All experiments were conducted within a single AWS availability zone (AZ), with SFTP segment transfers occurring between the orchestrating host and the launched EC2 nodes. Neither the AZ-internal network bandwidth nor the SFTP throughput were independently characterized or controlled. Variations in AZ-level congestion, TCP buffer tuning, or concurrent co-tenant network load may have introduced uncontrolled variability in the transfer phase, which is conflated with encoding time in the end-to-end wall-clock measurements. This limits the interpretability of any transfer-overhead hypotheses (e.g., the attributed $\approx$30\,s SFTP overhead in the chunk sensitivity analysis) and constitutes a threat to the internal validity of timing decompositions.

\textbf{Workload Representativeness and Chunk Size Generalizability:} The chunk size sensitivity analysis (Phase~4) was conducted on a single 10-minute, 1080p video representative of user-generated VOD content. The optimal 60-second chunk duration may not generalize to substantially different workload characteristics: shorter videos (e.g., $<2$ minutes) cannot be parallelized to 10 nodes with 60-second chunks; longer-form content (e.g., 2-hour films) may benefit from different chunk durations due to changed provisioning-to-encoding ratios; and videos with different complexity profiles (e.g., high-motion sports, animation) exhibit different per-second encoding times that shift the sensitivity curve. The identified optimal is therefore specific to the tested workload range and should be treated as a starting point for workload-specific calibration rather than a universal constant.

\textbf{ARM Graviton Homogeneity Scope:} The 100\% processor homogeneity observed for ARM Graviton instances was validated exclusively within \texttt{us-east-1} availability zones. AWS's custom silicon deployment policy may differ in other regions or newer AZ generations; the homogeneity claim should not be assumed to hold universally without independent validation in the target deployment region.

\textbf{Co-tenancy and Noisy Neighbor Effects:} All experiments were conducted on shared-tenancy on-demand EC2 instances, which are subject to co-tenancy variability. Noisy neighbor workloads from co-located VMs may introduce uncontrolled variance in CPU, memory bandwidth, and last-level cache availability. While the 30-iteration experimental design and the low observed coefficient of variation ($<$0.6\% for Phase~2 single-instance benchmarks) suggest that co-tenancy effects were negligible during measurement windows, their influence cannot be fully excluded without dedicated-tenancy replication. Deployments in production environments with strict latency SLAs should consider Dedicated Host or Dedicated Instance options to eliminate this source of variability.

\textbf{Generalizability:} Initial baseline experiments were heavily constrained to the \texttt{us-east-1} region, x86 architectures, and the H.264 codec. While our extended analysis successfully validated the horizontal scaling methodology across H.265/VP9 codecs, ARM Graviton processors, and the \texttt{sa-east-1} region, absolute economic claims must remain contextualized. Cloud provider pricing structures shift continuously, and hardware generations deprecate rapidly. Future research must expand upon these established provisioning optimization techniques to GPU-accelerated environments and complex multi-cloud architectural blends. Furthermore, our findings regarding the pronounced H.265 sequential performance gap on Graviton highlight an important consideration: relying on standard Linux repository binaries may substantially constrain ARM-native encoding performance. Future work should investigate deploying custom-compiled Docker images—expressly built with native NEON SIMD assembly instructions—to accurately evaluate the absolute theoretical limits of ARM encoders and circumvent software-hardware impedance. Finally, the temporal stability of these findings is not guaranteed: AWS continuously introduces new instance generations (e.g., M7, C7), retires older ones, and adjusts regional pricing. The identified sweet spots (e.g., \texttt{t3.micro} horizontal cluster at 60-second chunks) reflect pricing and hardware availability at the time of experimentation and should be re-validated periodically as the EC2 instance catalog evolves.
...
This study demonstrates that the primary limiting factor for cloud cost-efficiency in divisible video encoding is operational provisioning overhead rather than pure hardware throughput. Our comprehensive empirical analysis across x86 and ARM Graviton architectures---encompassing Burstable, General Purpose, and Compute Optimized instance families---quantified this bottleneck at up to 93.3\% of total processing time in unoptimized T-family horizontal deployments (Phase~3.1). 

By implementing an optimized custom AMI orchestration framework, we eliminated this setup latency, significantly altering scaling economics. In fully optimized parallel environments, distributed clusters of small instances (e.g., 10$\times$ \texttt{t3.micro}) consistently outperformed the processing capacity of single maximal instances (e.g., \texttt{c5.4xlarge}) across all evaluated families, under optimized AMI provisioning with 60-second chunks. For instance, the \texttt{t3.micro} cluster achieved an 89.4\% base-compute cost reduction relative to the \texttt{m5.4xlarge} in a cross-family comparison ($\approx$62\% under a realistic encoding-focused credit surcharge estimate, with the absolute worst case eliminating the advantage---see Section~\ref{limitations}), while simultaneously accelerating task completion.  

Our extended analysis demonstrated the necessity of careful architectural alignment: ARM Graviton instances demonstrated codec-specific parallel encoding advantages for H.265 workloads, with the \texttt{m7g.large} horizontal cluster achieving a median of 59.27s---a result comparable to x86 H.264 horizontal configurations, though the two involve different codecs and should not be interpreted as a direct architectural comparison. This median-level performance, however, coexists with high operational variance (CV~$>50\%$, driven by cloud provisioning jitter); deployments depending on consistent low-latency H.265 encoding on ARM should account for this variability before relying on median-only figures. Additionally, empirical chunk size sensitivity analysis identified the chunk duration that minimizes total wall-clock time, confirming the practical limits of parallel scaling across instance families. Finally, a holistic network cost assessment confirmed that upon maximizing computational efficiency, outbound internet egress emerges as the primary economic bottleneck for scalable VaaS pipelines.

Future research should extend this overhead-elimination methodology to GPU-accelerated instances and explore intelligent orchestration agents capable of dynamically tuning chunk sizes and managing cross-region egress optimizations. This work establishes an empirical foundation for cloud media processing, demonstrating that provisioning latency mitigation is a critical prerequisite to realizing the full cost-performance potential of horizontally scaled cloud architectures.
...
%\url{https://anonymous.4open.science/r/ec2sweetspot-blinded}.
%
...
%repository at \url{https://anonymous.4open.science/r/ec2sweetspot-blinded}, the
